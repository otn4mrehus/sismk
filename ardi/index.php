...
// FUNGSI UPLOAD YANG DIPERBAIKI - DENGAN DIREKTORI NAMA USER
function handle_file_upload($file, $jenis = 'arsip', $user_id = null, $allowed_types = ['pdf', 'doc', 'docx', 'jpg', 'jpeg', 'png']) {
    global $pdo;
    ...
    // PERBAIKAN: Buat folder berdasarkan tahun, bulan, dan nama user
    $tahun = date('Y');
    $bulan = date('m');
    
    // Buat path lengkap: base_path/tahun/bulan/nama_user/
    ...
    if (move_uploaded_file($file['tmp_name'], $target_path)) {
        // PERBAIKAN: Simpan path relatif untuk akses web
        $relative_path = 'uploads/' . ($jenis == 'photo' ? 'photo' : 'arsip') . '/' . $tahun . '/' . $bulan . '/' . $nama_user . '/' . $filename;
...
function get_file_path($filename, $jenis = 'arsip') {
    ...
    // Cari file di semua subdirectory tahun/bulan/user
...
// =============================================
// HANDLER FUNCTIONS - MANAJEMEN ARSIP (DENGAN SHARING)
// =============================================
...
// PERBAIKAN: Fungsi handle_upload dengan direktori nama user
function handle_upload() {
    global $pdo;
    check_access(['admin', 'kepsek', 'wakasek', 'kaprog', 'kabeng', 'guru']);
    ...
    if ($_SERVER['REQUEST_METHOD'] == 'POST') {
        ...
        $jenis_upload = $_POST['jenis_upload'] ?? 'file'; // Jenis upload: file atau link
        ...
        try {
            ...
            if ($jenis_upload == 'file') {
                ...
                $upload_result = handle_file_upload($_FILES['file'], 'arsip', $user_id);
...
// =============================================
// FUNGSI LAINNYA (users, kategori, profile, dll)
// =============================================
    ...
    if ($_SERVER['REQUEST_METHOD'] == 'POST') {
        ...
        try {
            ...
            if (!empty($_FILES['foto_profil']['name'])) {
                // PERBAIKAN: Upload file dengan jenis 'photo' dan user_id
