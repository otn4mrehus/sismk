...
// FUNGSI UTILITY
// =============================================
function get_jenis_by_kategori() {
    global $pdo;
    ...
    $kategori_id = $_GET['kategori_id'] ?? 0;
    ...
    try {
        $stmt = $pdo->prepare("SELECT id, nama_jenis FROM jenis_dokumen WHERE kategori_id = ? ORDER BY nama_jenis");
        ...
        echo json_encode([
            'success' => false,
            'error' => 'Gagal mengambil data jenis: ' . $e->getMessage()
        ]);
    }
    exit;
}
...
function handle_file_upload($file, $jenis = 'arsip', $user_id = null, $tahun_periode = null, $kategori_nama = null, $jenis_nama = null, $judul_dokumen = null, $allowed_types = ['pdf', 'doc', 'docx', 'jpg', 'jpeg', 'png']) {
    global $pdo;
    ...
    // PERBAIKAN: Struktur folder baru - uploads/arsip/[TAHUN]/[BULAN]/[NAMA_USER]/[KATEGORI]/[JENIS]/[JUDUL_DOKUMEN]/
    // Bersihkan nama kategori, jenis, dan judul untuk digunakan dalam path
    $kategori_folder = 'umum';
    $jenis_folder = 'lainnya';
    $judul_folder = 'unknown_title';
    ...
    if ($judul_dokumen) {
        $judul_folder = preg_replace('/[^a-zA-Z0-9]/', '_', $judul_dokumen);
        $judul_folder = strtolower(trim($judul_folder, '_'));
        // Batasi panjang folder judul untuk menghindari path terlalu panjang
        $judul_folder = substr($judul_folder, 0, 50);
    }
    
    // Buat path lengkap berdasarkan jenis
    if ($jenis == 'arsip') {
        $jenis_path = $base_path . $tahun . '/' . $bulan . '/' . $nama_user . '/' . $kategori_folder . '/' . $jenis_folder . '/' . $judul_folder . '/';
    } else {
    ...
    if (move_uploaded_file($file['tmp_name'], $target_path)) {
        ...
        if ($jenis == 'arsip') {
            $relative_path = 'uploads/arsip/' . $tahun . '/' . $bulan . '/' . $nama_user . '/' . $kategori_folder . '/' . $jenis_folder . '/' . $judul_folder . '/' . $filename;
        } else {
...
function initialize_database($pdo) {
    ...
    try {
        ...
        if ($check_admin == 0) {
            ...
            $default_categories = [
                ['Arsip Kelembagaan & Manajemen Sekolah', 'Dokumen terkait kelembagaan, manajemen, dan regulasi sekolah'],
                ['Arsip Persuratan & Ketatausahaan', 'Surat menyurat, agenda, dan administrasi umum'],
                ['Arsip Kepegawaian Guru & Tendik', 'Administrasi guru dan tenaga kependidikan'],
                ['Arsip Kesiswaan SMK', 'Data dan administrasi peserta didik'],
                ['Arsip Kurikulum, Mapel & Pembelajaran', 'Dokumen kurikulum dan kegiatan belajar mengajar'],
                ['Arsip Kompetensi Keahlian (Jurusan SMK)', 'Dokumen praktik kejuruan dan kompetensi keahlian'],
                ['Arsip PKL (Prakerin) / Dunia Industri', 'Dokumen PKL dan hubungan dengan DUDI'],
                ['Arsip Bursa Kerja Khusus (BKK)', 'Dokumen layanan ketenagakerjaan SMK'],
                ['Arsip UKK (Uji Kompetensi Keahlian)', 'Dokumen pelaksanaan UKK SMK'],
                ['Arsip Sarana & Prasarana SMK', 'Dokumen sarana, prasarana, dan lab praktik'],
                ['Arsip Keuangan Sekolah', 'RKAS, BOS, SPJ, audit dan keuangan sekolah'],
                ['Arsip Humas, Kemitraan, & Kegiatan Sekolah', 'Dokumen kegiatan sekolah dan kerjasama'],
                ['Arsip Penjaminan Mutu / Supervisi', 'Dokumen mutu pendidikan dan supervisi']
            ];
            ...
            $default_jenis = [
                // 1. Arsip Kelembagaan & Manajemen Sekolah
                [1, 'SK Pendirian SMK', 'Surat keputusan pendirian sekolah'],
                [1, 'SK Akreditasi', 'Dokumen akreditasi sekolah'],
                [1, 'SK Struktur Organisasi', 'Struktur organisasi & tupoksi'],
                [1, 'SK Kepala Sekolah & Wakil', 'Penetapan kepala sekolah & wakil'],
                [1, 'SK Komite Sekolah', 'SK pembentukan komite sekolah'],
                [1, 'Profil SMK', 'Visi, misi, dan profil sekolah'],
                [1, 'Peraturan Kepala Sekolah', 'Peraturan dan kebijakan resmi'],
            
                // 2. Arsip Persuratan & Ketatausahaan
                [2, 'Surat Masuk', 'Dokumen surat masuk'],
                [2, 'Surat Keluar', 'Dokumen surat keluar'],
                [2, 'Agenda Surat', 'Buku agenda surat masuk dan keluar'],
                [2, 'Surat Edaran', 'Informasi edaran resmi sekolah'],
                [2, 'Surat Tugas', 'Surat penugasan internal'],
                [2, 'Nota Dinas', 'Nota dinas internal'],
                [2, 'Undangan Rapat', 'Surat undangan kegiatan'],
                [2, 'Berita Acara', 'Dokumen berita acara dan disposisi'],
            
                // 3. Arsip Kepegawaian Guru & Tendik
                [3, 'SK Guru Tetap / Honorer', 'Penetapan guru tetap atau honorer'],
                [3, 'SK Penugasan Guru / Wali Kelas', 'Penugasan guru dan wali kelas'],
                [3, 'SK Pembina Ekstrakurikuler', 'SK pembina ekskul'],
                [3, 'SK Kenaikan Pangkat', 'Kenaikan pangkat pegawai'],
                [3, 'SK Mutasi', 'Mutasi guru atau tendik'],
                [3, 'Riwayat Hidup Guru', 'Biodata dan dokumen riwayat guru'],
                [3, 'PKG', 'Penilaian kinerja guru'],
                [3, 'Absensi Pegawai', 'Absensi guru dan tendik'],
            
                // 4. Arsip Kesiswaan SMK
                [4, 'Buku Induk Siswa', 'Data induk peserta didik'],
                [4, 'Dokumen PPDB', 'Arsip penerimaan peserta didik baru'],
                [4, 'Surat Pernyataan Siswa', 'Surat pernyataan yang ditandatangani siswa'],
                [4, 'Data Pelanggaran', 'Data pelanggaran dan pembinaan'],
                [4, 'Data Prestasi', 'Prestasi akademik dan non-akademik siswa'],
                [4, 'Data Beasiswa', 'Penerima dan persyaratan beasiswa'],
                [4, 'Arsip Organisasi Siswa', 'Arsip OSIS, Pramuka, Paskibra, PMR'],
            
                // 5. Arsip Kurikulum, Mapel & Pembelajaran
                [5, 'Dokumen KOS SMK', 'Dokumen kurikulum operasional SMK'],
                [5, 'Struktur Kurikulum Jurusan', 'Struktur per kompetensi keahlian'],
                [5, 'RPP / Modul Ajar / ATP', 'Perangkat pembelajaran'],
                [5, 'Prota & Promes', 'Program tahunan dan semester'],
                [5, 'Jadwal Pelajaran', 'Jadwal kegiatan belajar mengajar'],
                [5, 'Daftar Nilai', 'Data nilai siswa'],
                [5, 'Dokumen Asesmen / Raport', 'Laporan hasil belajar'],
                [5, 'LKPD / Bahan Ajar', 'Bahan ajar atau modul siswa'],
            
                // 6. Arsip Kompetensi Keahlian (Jurusan)
                [6, 'RPS / Proyek Keahlian', 'Rencana proyek per jurusan'],
                [6, 'Standar Industri', 'Standar kompetensi dari industri'],
                [6, 'Daftar Mitra Industri', 'Industri yang bekerjasama'],
                [6, 'Dokumen Teaching Factory', 'Dokumen pelaksanaan TEFA'],
                [6, 'Proyek Kreatif', 'Rencana dan laporan proyek kreatif'],
                [6, 'Peralatan Praktik', 'Inventaris peralatan praktik'],
            
                // 7. Arsip PKL / DUDI
                [7, 'Proposal PKL', 'Proposal pengajuan prakerin'],
                [7, 'Surat Pengajuan PKL', 'Surat permohonan PKL ke industri'],
                [7, 'Surat Pengantar PKL / SPPD', 'Surat pengantar siswa ke DUDI'],
                [7, 'Surat Balasan DUDI', 'Respon atau persetujuan industri'],
                [7, 'Jurnal Harian PKL', 'Jurnal kegiatan harian'],
                [7, 'Monitoring PKL', 'Lembar monitoring pembimbing'],
                [7, 'Penilaian Industri', 'Nilai dan evaluasi dari industri'],
                [7, 'Sertifikat PKL', 'Sertifikat hasil PKL'],
                [7, 'Berkas Sidang PKL', 'Dokumen untuk sidang laporan PKL'],
            
                // 8. Arsip Bursa Kerja Khusus (BKK)
                [8, 'Data Alumni', 'Data lulusan SMK'],
                [8, 'Data Pencari Kerja', 'Data lulusan pencari kerja'],
                [8, 'Mitra Penyedia Lowongan', 'Kerjasama perusahaan'],
                [8, 'Informasi Rekrutmen', 'Pengumuman lowongan kerja'],
                [8, 'Tracer Study', 'Laporan jejak alumni bekerja/kuliah'],
            
                // 9. Arsip UKK (Uji Kompetensi Keahlian)
                [9, 'SK Panitia UKK', 'Penetapan panitia UKK'],
                [9, 'SK Penguji UKK', 'Penguji internal & eksternal'],
                [9, 'Paket Soal UKK', 'Soal uji kompetensi'],
                [9, 'Berita Acara UKK', 'BA pelaksanaan UKK'],
                [9, 'Nilai UKK', 'Hasil penilaian UKK'],
                [9, 'Sertifikat UKK', 'Sertifikat lulus UKK'],
            
                // 10. Arsip Sarana & Prasarana SMK
                [10, 'Dokumen Pengadaan', 'Pengadaan barang dan lab'],
                [10, 'Inventaris Aset', 'Inventaris aset sekolah'],
                [10, 'BA Serah Terima', 'Berita acara serah terima'],
                [10, 'Pemeliharaan Sarpras', 'Laporan pemeliharaan fasilitas'],
                [10, 'Dokumen Lab', 'Dokumen ruang praktik dan laboratorium'],
            
                // 11. Arsip Keuangan Sekolah
                [11, 'RKAS / RKAM', 'Rencana anggaran sekolah'],
                [11, 'Arsip BOS / BOP', 'Berkas BOS / BOP'],
                [11, 'SPJ & Kwitansi', 'Pertanggungjawaban keuangan'],
                [11, 'Laporan Realisasi', 'Realisasi anggaran'],
                [11, 'Pengeluaran Kegiatan', 'Catatan biaya kegiatan'],
                [11, 'Audit Keuangan', 'Berkas audit keuangan'],
            
                // 12. Arsip Humas, Kemitraan, & Kegiatan Sekolah
                [12, 'SK Kepanitiaan', 'Penetapan panitia kegiatan'],
                [12, 'Proposal Kegiatan', 'Rencana kegiatan sekolah'],
                [12, 'Notulen Rapat', 'Catatan rapat resmi'],
                [12, 'Agenda Kegiatan', 'Jadwal dan dokumentasi kegiatan'],
                [12, 'MOU / PKS', 'Kerjasama sekolah dengan industri/instansi'],
                [12, 'Dokumentasi Kegiatan', 'Foto dan bukti kegiatan'],
            
                // 13. Arsip Penjaminan Mutu / Supervisi
                [13, 'Instrumen PMP / EDS', 'Pemantauan mutu pendidikan'],
                [13, 'Supervisi Akademik', 'Laporan supervisi pembelajaran'],
                [13, 'Monitoring & Evaluasi', 'Dokumen Monev kegiatan'],
                [13, 'Sasaran Mutu', 'Dokumen capaian mutu'],
                [13, 'Program Kerja Tahunan', 'Dokumen program dan laporan tahunan']
            ];
...
switch($action) {
    ...
    case 'get_download_monthly_stats':
        ...
        break;
    case 'get_jenis_by_kategori':
        get_jenis_by_kategori();
        break;    
    default:
        show_dashboard();
}
...
<div id="uploadModal" class="modal">
        ...
        <div class="modal-body">
            <form id="uploadForm" enctype="multipart/form-data" method="POST" action="index.php?action=upload">
                ...
                <div class="form-row">
                    <div class="form-group">
                        <label>Kategori *</label>
                        <select name="kategori_id" id="kategori_id" required onchange="loadJenisByKategori(this.value)">
                ...
                <div class="form-group">
                    ...
                    <div style="display: flex; gap: 20px; margin-top: 8px;">
                        <label style="display: flex; align-items: center; gap: 5px;">
                            ...
                            <span style="color:blue; font-size: 0.9em;">Upload File ke Server</span>
                        </label>
                        <label style="display: flex; align-items: center; gap: 5px;">
                            <input type="radio" name="jenis_upload" value="link" onchange="toggleUploadType()">
                            <span style="color:grey; font-size: 0.9em;">Kirim Link Google Drive/Cloud</span>
                        </label>
                    </div>
                </div>
                ...
                <div id="fileUploadSection">
                    <div class="form-group">
                        <label>File Dokumen * (Maksimal 25 file)</label>
                        <input type="file" name="files[]" id="fileInput" multiple accept=".pdf,.doc,.docx,.jpg,.jpeg,.png">
                        <small class="form-help">Format yang didukung: PDF, DOC, DOCX, JPG, JPEG, PNG (Max 10MB per file). Anda dapat memilih hingga 5 file.</small>
    ...
    <script>
    // Fungsi untuk menangani multi file upload
    function handleMultiFileSelect(event) {
        ...
        if (fileInput.files.length > maxFiles) {
            alert(`Maksimal ${maxFiles} file yang diizinkan. Silakan pilih ulang.`);
            fileInput.value = '';
            return;
        }
        ...
        Array.from(fileInput.files).forEach((file, index) => {
            ...
            fileItem.innerHTML = `
                <div>
                    ...
                    <span>${file.name}</span>
                    <small style="color: #6c757d; margin-left: 8px;">(${formatFileSize(file.size)})</small>
                </div>
                <button type="button" class="btn btn-sm btn-danger" onclick="removeFile(${index})" style="padding: 2px 6px;">
    ...
    // Fungsi untuk menghapus file dari list
    function removeFile(index) {
        ...
        // Update display
        handleMultiFileSelect({ target: fileInput });
    }
    ...
    // Event listener untuk multi file
    document.addEventListener('DOMContentLoaded', function() {
        const fileInput = document.getElementById('fileInput');
        if (fileInput) {
    ...
    // Fungsi untuk memuat jenis dokumen berdasarkan kategori
    function loadJenisByKategori(kategoriId, selectedJenisId = '') {
        const jenisSelect = document.getElementById('jenis_id');
        const editJenisSelect = document.getElementById('edit_jenis_id');
        
        if (!kategoriId) {
            // Reset jika tidak ada kategori yang dipilih
            if (jenisSelect) {
                jenisSelect.innerHTML = '<option value="">Pilih Kategori terlebih dahulu</option>';
                jenisSelect.classList.remove('loading-jenis');
            }
            if (editJenisSelect) {
                editJenisSelect.innerHTML = '<option value="">Pilih Kategori terlebih dahulu</option>';
                editJenisSelect.classList.remove('loading-jenis');
            }
            return;
        }
        
        // AJAX request untuk mengambil jenis dokumen berdasarkan kategori
        fetch(`index.php?action=get_jenis_by_kategori&kategori_id=${kategoriId}`)
            .then(response => response.json())
            .then(data => {
                // Remove loading class
                if (jenisSelect) jenisSelect.classList.remove('loading-jenis');
                if (editJenisSelect) editJenisSelect.classList.remove('loading-jenis');
                
                if (data.success && data.jenis.length > 0) {
                    const optionsHtml = data.jenis.map(jenis => 
                        `<option value="${jenis.id}" ${selectedJenisId == jenis.id ? 'selected' : ''}>${jenis.nama_jenis}</option>`
                    ).join('');
                    
                    if (jenisSelect) {
                        jenisSelect.innerHTML = '<option value="">Pilih Jenis</option>' + optionsHtml;
                    }
                    if (editJenisSelect) {
                        editJenisSelect.innerHTML = '<option value="">Pilih Jenis</option>' + optionsHtml;
                    }
                } else {
                    const noDataOption = '<option value="">Tidak ada jenis dokumen</option>';
                    if (jenisSelect) jenisSelect.innerHTML = noDataOption;
                    if (editJenisSelect) editJenisSelect.innerHTML = noDataOption;
                }
            })
            .catch(error => {
                ...
                if (jenisSelect) {
                    jenisSelect.classList.remove('loading-jenis');
                    jenisSelect.innerHTML = '<option value="">Error memuat data</option>';
                }
                if (editJenisSelect) {
                    editJenisSelect.classList.remove('loading-jenis');
                    editJenisSelect.innerHTML = '<option value="">Error memuat data</option>';
                }
            });
    }

    // Event listener untuk perubahan kategori pada form upload
    document.addEventListener('DOMContentLoaded', function() {
        ...
        function handleKategoriChange(selectElement, targetJenisSelect) {
            const kategoriId = selectElement.value;
            
            if (targetJenisSelect) {
                targetJenisSelect.classList.add('loading-jenis');
                targetJenisSelect.innerHTML = '<option value="">Memuat jenis dokumen...</option>';
            }
            
            loadJenisByKategori(kategoriId);
            
            // Remove loading class setelah selesai (ditangani di loadJenisByKategori)
        }
        
        if (kategoriSelect && jenisSelect) {
            kategoriSelect.addEventListener('change', function() {
                handleKategoriChange(this, jenisSelect);
            });
        }
        
        if (editKategoriSelect && editJenisSelect) {
            editKategoriSelect.addEventListener('change', function() {
                handleKategoriChange(this, editJenisSelect);
            });
        }
    });
        ...
        fetch(`index.php?action=get_file_data&id=${fileId}`)
            ...
            .then(data => {
                if (data.success) {
                    ...
                    // Load jenis dokumen berdasarkan kategori
                    if (data.kategori_id) {
                        loadJenisByKategori(data.kategori_id, data.jenis_id);
                    }
    ...
    /* Styles untuk multi file upload */
.file-list {
    max-height: 200px;
    overflow-y: auto;
}
...
/* Responsive design untuk multi upload */
@media (max-width: 768px) {
    .file-item {
        flex-direction: column;
    ...
    <div class="upload-structure-info">
        ...
        <div class="structure-path">
            <h6 style="font-size:0.9em; color:#FF0000;"> uploads/arsip/[TAHUN]/[BULAN]/[NAMA_USER]/[KATEGORI]/[JENIS]/[JUDUL_DOKUMEN]/[FILE]</h6>        </div>
...
// PERBAIKAN: Fungsi handle_upload dengan struktur folder baru dan multi upload
    ...
    if ($_SERVER['REQUEST_METHOD'] == 'POST') {
        ...
        try {
            ...
            if ($jenis_upload == 'file') {
                // UPLOAD MULTI FILE KE SERVER
                if (empty($_FILES['files']['name'][0])) {
                    $_SESSION['error'] = "Harap pilih minimal satu file untuk diupload";
                    header('Location: index.php?action=arsip');
                    exit;
                }
                ...
                if ($file_count > 25) {
                    $_SESSION['error'] = "Maksimal 25 file yang dapat diupload sekaligus";
                    header('Location: index.php?action=arsip');
                    exit;
                }
                ...
                for ($i = 0; $i < $file_count; $i++) {
                    if ($_FILES['files']['error'][$i] !== UPLOAD_ERR_OK) {
                        $error_messages[] = "Error dalam upload file: " . $_FILES['files']['name'][$i];
                        continue;
                    }
                    
                    // Create file array for handle_file_upload function
                    $file = [
                    ...
                    // PERBAIKAN: Kirim tahun_periode, kategori_nama, jenis_nama, dan judul ke fungsi handle_file_upload
                    $upload_result = handle_file_upload($file, 'arsip', $user_id, $tahun_periode, $kategori_nama, $jenis_nama, $judul);
                    ...
                    if ($upload_result['success']) {
                        ...
                        // Insert dokumen untuk setiap file
                        ...
                        $stmt->execute([
                            $judul . ' - ' . $nama_file_asli, // Tambahkan nama file ke judul untuk membedakan
                            $deskripsi,
                            $nama_file,
                            $nama_file_asli,
                            $ukuran_file,
                        ...
                        // Jika akses adalah shared, simpan data sharing untuk setiap dokumen
                        if ($akses == 'shared' && !empty($shared_users)) {
                            $stmt_share = $pdo->prepare("INSERT INTO dokumen_sharing (dokumen_id, user_id, can_edit) VALUES (?, ?, ?)");
                            
                            foreach ($shared_users as $shared_user_id) {
                                $edit_permission = isset($can_edit[$shared_user_id]) ? 1 : 0;
                                $stmt_share->execute([$dokumen_id, $shared_user_id, $edit_permission]);
                            }
                        }
                        
                        $success_count++;
                        $uploaded_files[] = $nama_file_asli;
                        
                    } else {
                        $error_messages[] = $upload_result['error'] . " (File: " . $_FILES['files']['name'][$i] . ")";
                    }
                }
                
                if ($success_count > 0) {
                    $pdo->commit();
                    log_audit($user_id, 'UPLOAD_MULTI', "Upload $success_count dokumen: " . implode(', ', $uploaded_files));
                    $_SESSION['success'] = "Berhasil mengupload $success_count file!";
                    
                    if (!empty($error_messages)) {
                        $_SESSION['info'] = "Beberapa file gagal diupload: " . implode('; ', $error_messages);
                    }
                } else {
                    $pdo->rollBack();
                    $_SESSION['error'] = "Gagal mengupload semua file: " . implode('; ', $error_messages);
                }
                
            } else {
                // UPLOAD LINK GOOGLE DRIVE (tetap sama)
                if (empty($file_url)) {
                ...
                log_audit($user_id, 'UPLOAD', "Upload dokumen link: $judul");
                $_SESSION['success'] = "Dokumen link berhasil diupload!";
            }
            
        } catch(PDOException $e) {
            $pdo->rollBack();
            // Delete uploaded files if database insert fails (hanya untuk upload file)
            if ($jenis_upload == 'file' && isset($upload_result) && !empty($uploaded_files)) {
                foreach ($uploaded_files as $uploaded_file) {
                    $file_path = get_file_path($uploaded_file, 'arsip');
                ...
                <table class="data-table">
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td style="color:grey; font-size:0.7em;"><?= htmlspecialchars($kategori['deskripsi']) ?></td>
                            <td>
                                <div class="btn-group">
                ...
                <table class="data-table">
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td style="color:grey; font-size:0.7em;"><?= htmlspecialchars($jenis['nama_kategori']) ?></td>
                            <td style="color:grey; font-size:0.7em;"><?= htmlspecialchars($jenis['deskripsi']) ?></td>
                            <td>
                                <div class="btn-group">
    ...
    if ($_SERVER['REQUEST_METHOD'] == 'POST') {
        ...
        try {
            ...
            if (!empty($_FILES['foto_profil']['name'])) {
                // PERBAIKAN: Upload file dengan jenis 'photo' dan user_id, tanpa parameter tambahan
...
function show_login_page() {
    ...
    <head>
        <meta charset="UTF-8">
        <meta name="viewport" content="width=device-width, initial-scale=1.0">
        <title>Login - SiArDi SMKN 6 Kota Serang</title>
        <link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.0.0/css/all.min.css">
        <style>
            * {
                margin: 0;
    ...
    <body>
        <div class="login-container">
            <div class="login-header">
                <h1><i class="fas fa-archive"></i> SiArDi</h1>
...
function show_header($title = 'Dashboard') {
    ...
    <head>
        <meta charset="UTF-8">
        <meta name="viewport" content="width=device-width, initial-scale=1.0">
        <title><?= htmlspecialchars($title) ?> - SiArDi</title>
        <link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.0.0/css/all.min.css">
        <style>
            * {
                margin: 0;
        ...
        <div class="app-container">
            ...
            <div class="sidebar" id="sidebar">
                <div class="sidebar-header">
                    <h2><i class="fas fa-archive"></i> SiArDi</h2>
                    <p>Sistem Arsip Digital</p>
                </div>
...
function manage_pengaturan_modul() {
    ...
    <div class="module-settings-container">
        ...
        <div class="alert alert-info" style="font-size:0.9em;">
            <h4> Sistem Pengaturan Baru <i class="fas fa-info-circle"></i></h4>
            <p><strong>Perhatian:</strong><i> Sistem sekarang hanya menggunakan pengaturan modul menu. Tidak ada akses default berdasarkan level user.</i></p>
            <p><i>Setiap user (kecuali admin) harus diatur akses modulnya secara manual melalui tabel di bawah.</i></p>
        </div>
        ...
        <div class="modules-section">
            <div class="section-header">
                ...
                <p style="padding-left:25px;">Semua modul yang tersedia dalam sistem</p>
            </div>
            <div class="modules-grid">
                <?php foreach($modules as $module): ?>
                <div class="module-card">
                    <div class="module-header">
                        <h4 style="padding-right:10px;"><?= htmlspecialchars($module['module_description']) ?></h4>
                        <span  class="badge badge-<?= get_level_badge($module['required_level']) ?> module-badge">
                            Level: <?= ucfirst($module['required_level']) ?>
                        </span>
                    </div>
                    <div class="module-info">
                        <p style="font-size:0.7em;"><strong><i class="fas fa-code"></i> Kode Modul:</strong> <?= htmlspecialchars($module['module_name']) ?></p>
                        <p style="font-size:0.7em;"><strong><i class="fas fa-info-circle"></i> Status Sistem:</strong> 
                            <span class="badge <?= $module['is_active'] ? 'badge-success' : 'badge-danger' ?>">
                                <?= $module['is_active'] ? 'Aktif' : 'Nonaktif' ?>
                            </span>
                        </p>
                        <p style="font-size:0.7em;"><strong><i class="fas fa-align-left"></i> Level Default (Referensi):</strong> <?= ucfirst($module['required_level']) ?></p>
                    </div>
                </div>
                <?php endforeach; ?>
            </div>
