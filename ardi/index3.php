...
function show_dashboard() {
    ...
    if ($user_level == 'admin') {
        ...
        // REVISI: Hitung total kategori dan jenis untuk admin
        $total_kategori = $pdo->query("SELECT COUNT(DISTINCT kategori_id) FROM dokumen WHERE kategori_id IS NOT NULL")->fetchColumn();
        $total_jenis = $pdo->query("SELECT COUNT(DISTINCT jenis_id) FROM dokumen WHERE jenis_id IS NOT NULL")->fetchColumn();
    } else {
        ...
        // REVISI: Hitung total kategori dan jenis untuk user biasa
        // Hitung kategori
        $stmt = $pdo->prepare("SELECT COUNT(DISTINCT d.kategori_id) 
                              FROM dokumen d 
                              LEFT JOIN dokumen_sharing ds ON d.id = ds.dokumen_id 
                              WHERE (d.user_id = ? OR d.akses = 'public' OR (d.akses = 'shared' AND ds.user_id = ?))
                              AND d.kategori_id IS NOT NULL");
        $stmt->execute([$user_id, $user_id]);
        $total_kategori = $stmt->fetchColumn();

        // Hitung jenis
        $stmt = $pdo->prepare("SELECT COUNT(DISTINCT d.jenis_id) 
                              FROM dokumen d 
                              LEFT JOIN dokumen_sharing ds ON d.id = ds.dokumen_id 
                              WHERE (d.user_id = ? OR d.akses = 'public' OR (d.akses = 'shared' AND ds.user_id = ?))
                              AND d.jenis_id IS NOT NULL");
        $stmt->execute([$user_id, $user_id]);
        $total_jenis = $stmt->fetchColumn();
    }
    ...
    <div class="dashboard">
        ...
        <hr style="border:none;margin-top:20px;">
        <div class="stats-grid">
            ...
            <!-- REVISI: Tambahkan statistik Kategori Arsip -->
            <div class="stat-card">
                <div class="stat-icon">
            ...
            <!-- REVISI: Tambahkan statistik Jenis Arsip -->
            <div class="stat-card">
                <div class="stat-icon">
        ...
        <div class="dashboard-content">
            ...
            <hr style="border:none;margin-top:30px;">
...
// ... (Fungsi-fungsi lainnya tetap sama seperti pada kode sebelumnya)
// Karena panjang kode, saya akan lanjutkan dengan fungsi-fungsi yang tersisa
// yang tidak mengalami perubahan signifikan
