...
function getPhotos($filterPrivate = true) {
    ...
    if ($filterPrivate && !isset($_SESSION['user_id'])) {
        ...
        $sql .= " WHERE (is_private = 0 OR (is_private = 1 AND user_id = ?))";
...
function getUserAlbums($userId = null) {
    ...
    if ($role === 'admin' || $role === 'superadmin') {
        return dbGetAll("SELECT DISTINCT album_id, album_title FROM photos WHERE album_id IS NOT NULL AND album_id != '' ORDER BY album_title");
    }
    
    if ($userId) {
        return dbGetAll("SELECT DISTINCT album_id, album_title FROM photos WHERE user_id = ? AND album_id IS NOT NULL AND album_id != '' ORDER BY album_title", [$userId]);
    }
...
<body>
    ...
    <nav class="navbar navbar-expand-lg navbar-dark sticky-top">
        <div class="container">
            <a class="navbar-brand" href="?action=home">
                <i class="fas fa-camera-retro me-2"></i>Galeri Kegiatan SMKN 6 Kota Serang
            </a>
    ...
    // Sort by album_sort_order DESC, then created_at DESC (higher sort order = first/top)
    ...
    <section class="py-5" style="background: #f8fafc; min-height: 80vh;">
        <div class="container">
            ...
            <?php else: ?>
            <?php 
            // Group photos by album/batch for toggle view
            $groupedPhotos = [];
            foreach ($myPhotos as $p) {
                ...
                <div class="card-body p-0">
                    <div id="groupedView">
                        ...
                        <table class="table table-hover mb-0 group-table" id="group-<?= $groupKey ?>" style="display: <?= in_array($groupKey, $expandedGroupsState) ? 'table' : 'none' ?>;">
                            <thead class="table-light">
                                <tr>
                                    <th class="px-3" style="width: 40px;"><input type="checkbox" class="form-check-input" onchange="toggleSelectAllInGroup('<?= $groupKey ?>', this)"></th>
                                    <th class="px-3">Foto</th>
                                    <th>Judul</th>
                                    <th>Kategori</th>
                                    <th>Status</th>
                            ...
                            <tbody>
                                ...
                                <tr data-photo-id="<?= $photo['id'] ?>">
                                    ...
                                    <td>
                                        <div class="inline-edit" data-field="title" data-id="<?= $photo['id'] ?>">
                                            <strong class="photo-title-display" onclick="event.stopPropagation(); toggleMyInlineEditField(<?= $photo['id'] ?>, 'title')"><?= htmlspecialchars($photo['title']) ?></strong>
                                        ...
                                        <div class="small text-success"><i class="fas fa-tag me-1"></i><?= htmlspecialchars($photo['photo_label']) ?></div>
                                        <?php endif; ?>
                                        <small class="text-muted"><?= htmlspecialchars($photo['description'] ?: '-') ?></small>
                                    </td>
                                    <td>
                                        <div class="inline-edit" data-field="category" data-id="<?= $photo['id'] ?>">
                                            <small class="photo-category-display" onclick="event.stopPropagation(); toggleMyInlineEditField(<?= $photo['id'] ?>, 'category')">
                                                <?php $catNames = []; foreach ($photoCats as $catId) { foreach ($categories as $cat) { if ($cat['id'] == $catId) { $catNames[] = $cat['name']; break; } } } echo htmlspecialchars(implode(', ', $catNames) ?: 'Lainnya'); ?>
                                            </small>
                                            <select class="form-select form-select-sm edit-input" style="display:none; width: auto;" onchange="saveMyInlineEditField(this, 'category', <?= $photo['id'] ?>)">
                                                <?php foreach ($categories as $cat): ?>
                                                <option value="<?= $cat['id'] ?>" <?= in_array($cat['id'], $photoCats) ? 'selected' : '' ?>><?= htmlspecialchars($cat['name']) ?></option>
                                                <?php endforeach; ?>
                                            </select>
                                        </div>
                                    </td>
                                    <td>
                                        <div class="inline-edit" data-field="is_private" data-id="<?= $photo['id'] ?>">
                                            <span class="photo-status-display cursor-pointer" onclick="event.stopPropagation(); toggleMyInlineEditField(<?= $photo['id'] ?>, 'is_private')">
                                                <?php if ($photo['is_private'] == 1): ?><span class="badge bg-danger"><i class="fas fa-lock"></i> Private</span><?php else: ?><span class="badge bg-success"><i class="fas fa-globe"></i> Public</span><?php endif; ?>
                                            </span>
                                            <select class="form-select form-select-sm edit-input" style="display:none; width: auto;" onchange="event.stopPropagation(); quickSaveStatus(this, <?= $photo['id'] ?>)">
                                                <option value="0" <?= $photo['is_private'] == 0 ? 'selected' : '' ?>>Public</option>
                                                <option value="1" <?= $photo['is_private'] == 1 ? 'selected' : '' ?>>Private</option>
                                            </select>
                                        </div>
                                    ...
                                    <td class="text-end px-3">
                                        <div class="btn-group btn-group-sm">
                                            <button class="btn btn-outline-secondary" title="Pindah ke atas" onclick="reorderPhoto(<?= $photo['id'] ?>, 'up')" <?= $isFirst ? 'disabled' : '' ?>><i class="fas fa-chevron-up"></i></button>
                                            <button class="btn btn-outline-secondary" title="Pindah ke bawah" onclick="reorderPhoto(<?= $photo['id'] ?>, 'down')" <?= $isLast ? 'disabled' : '' ?>><i class="fas fa-chevron-down"></i></button>
                                            <button class="btn btn-outline-primary" title="Edit Inline" onclick="toggleMyInlineEdit(<?= $photo['id'] ?>)"><i class="fas fa-edit"></i></button>
                                            <form method="POST" class="d-inline" onsubmit="return confirm('Hapus foto ini?')">
                                                <input type="hidden" name="csrf_token" value="<?= $csrfToken ?>">
                                                <input type="hidden" name="photo_id" value="<?= $photo['id'] ?>">
                                                <button type="submit" name="delete_photo" class="btn btn-outline-danger" title="Hapus"><i class="fas fa-trash"></i></button>
                                            </form>
                                        </div>
                                    </td>
                                </tr>
                            ...
                            <tbody>
                                ...
                                <tr data-photo-id="<?= $photo['id'] ?>" class="photo-row">
                                    ...
                                    <td class="text-end px-3">
                                        <div class="btn-group btn-group-sm">
                                            <?php 
                                            $photoBatchId = $photo['batch_id'] ?? 'single_' . $photo['id'];
                                            $batchPhotos = $myPhotoBatches[$photoBatchId] ?? [];
                                            $batchIds = array_column($batchPhotos, 'id');
                                            $currentIndex = array_search($photo['id'], $batchIds);
                                            $isFirstInBatch = $currentIndex === 0;
                                            $isLastInBatch = $currentIndex === count($batchIds) - 1;
                                            ?>
                                            <button class="btn btn-outline-secondary" title="Pindah ke atas" onclick="reorderPhoto(<?= $photo['id'] ?>, 'up')" <?= $isFirstInBatch ? 'disabled' : '' ?>>
                                                <i class="fas fa-chevron-up"></i>
                                            </button>
                                            <button class="btn btn-outline-secondary" title="Pindah ke bawah" onclick="reorderPhoto(<?= $photo['id'] ?>, 'down')" <?= $isLastInBatch ? 'disabled' : '' ?>>
    ...
    <script>
            ...
            .then(function(data) {
                ...
                if (data.success) {
                    ...
                    if (field === 'photo_label') {
                        ...
                        if (display) {
                            if (value === 'new_album') {
                                value = 'Album Baru ' + new Date().toLocaleDateString('id-ID');
                            }
                            display.innerHTML = '<i class="fas fa-folder me-1"></i>' + value;
                        }
                    } else {
                        display = row.querySelector('.photo-' + field + '-display');
                    }
                    if (display && field !== 'album_id') {
                        if (field === 'photo_label') {
        ...
        function toggleMyInlineEdit(photoId) {
            ...
            var inputs = row.querySelectorAll('input.edit-input, select.edit-input');
            var displays = row.querySelectorAll('.photo-title-display, .photo-label-display, .photo-category-display, .photo-status-display, .photo-album-display');
        ...
        function toggleMyInlineEditField(photoId, field) {
            var row = document.querySelector('.group-table tr[data-photo-id="' + photoId + '"]');
            if (!row) row = document.querySelector('#myPhotosTable tr[data-photo-id="' + photoId + '"]');
            if (!row) return;
            
            var fieldContainer = row.querySelector('.inline-edit[data-field="' + field + '"]');
            if (!fieldContainer) return;
            
            var display = fieldContainer.querySelector('.photo-title-display, .photo-category-display, .photo-status-display');
        ...
        function saveMyInlineEditField(input, field, photoId) {
            ...
            if (!display) display = row.querySelector('.photo-title-display');
            if (!display) display = row.querySelector('.photo-category-display');
            if (!display) display = row.querySelector('.photo-status-display');
            if (!display && field === 'is_private') display = row.querySelector('.photo-status-display');
            
            var btn = input;
            if (field === 'is_private') {
                var container = row.querySelector('.inline-edit[data-field="is_private"]');
                if (container) {
                    display = container.querySelector('.photo-status-display');
                    btn = container.querySelector('select');
                }
            }
