...
function getUserAlbums($userId = null) {
    ...
    if ($userId) {
        return dbGetAll("SELECT album_id, album_title, MAX(created_at) as latest_photo FROM photos WHERE user_id = ? AND album_id IS NOT NULL AND album_id != '' GROUP BY album_id, album_title ORDER BY latest_photo DESC", [$userId]);
    }
