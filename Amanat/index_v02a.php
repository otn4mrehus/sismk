...
if ($action && isset($_SESSION['user_id'])) {
    ...
    // ========== CATEGORIES (diizinkan untuk semua user) ==========
    ...
    // ========== BRANDS (diizinkan untuk semua user) ==========
    ...
    // ========== TYPES (diizinkan untuk semua user) ==========
    ...
    if ($action == 'getItems' && $method == 'GET') {
        ...
        if ($type_id > 0) {
            $sql .= " AND items.type_id = ?";
            $countSql .= " AND items.type_id = ?";
            $params[] = $type_id;
            $types .= 'i';
        }
        if (!empty($search)) {
            $sql .= " AND (items.name LIKE ? OR items.item_code LIKE ? OR brands.name LIKE ? OR categories.name LIKE ? OR types.name LIKE ?)";
            $countSql .= " AND (items.name LIKE ? OR items.item_code LIKE ? OR brands.name LIKE ? OR categories.name LIKE ? OR types.name LIKE ?)";
            $searchTerm = "%$search%";
    ...
    // ----- GET ITEM BY CODE (manual entry) -----
    if ($action == 'getItemByCode' && $method == 'GET') {
        $code = $_GET['code'] ?? '';
        $stmt = $conn->prepare("SELECT items.*, categories.name as category_name, brands.name as brand_name, types.name as type_name
                                FROM items 
                                LEFT JOIN categories ON items.category_id = categories.id 
    ...
    // ----- RETURN ITEM (dari halaman pengembalian) -----
    ...
    // ----- RETURN TRANSACTION ITEM (dari halaman riwayat, untuk item individual) -----
    if ($action == 'returnTransactionItem' && $method == 'POST') {
        if (!$isAdmin && !$isOperator) { http_response_code(403); echo json_encode(['error' => 'Forbidden']); exit; }
        
        $input = getInput();
        $transaction_item_id = $input['transaction_item_id'];
        $return_signature = $input['return_signature'] ?? null;
        $return_photo = null; // nanti bisa ditambahkan upload jika perlu
        
        $conn->begin_transaction();
        try {
            // Ambil data transaction_item
            $stmt = $conn->prepare("SELECT ti.*, i.id as item_id FROM transaction_items ti JOIN items i ON ti.item_id = i.id WHERE ti.id = ?");
            $stmt->bind_param('i', $transaction_item_id);
            $stmt->execute();
            $ti = $stmt->get_result()->fetch_assoc();
            if (!$ti) throw new Exception('Item transaksi tidak ditemukan');
            if ($ti['status'] == 'returned') throw new Exception('Item sudah dikembalikan');

            // Update transaction_items
            $upd = $conn->prepare("UPDATE transaction_items 
                                   SET status = 'returned', return_date = NOW(), return_signature = ? 
                                   WHERE id = ?");
            $upd->bind_param('si', $return_signature, $transaction_item_id);
            $upd->execute();

            $upd2 = $conn->prepare("UPDATE items SET status = 'available' WHERE id = ?");
            $upd2->bind_param('i', $ti['item_id']);
            $upd2->execute();
    ...
    // ----- REVERT TRANSACTION ITEM (mengembalikan item yang sudah dikembalikan menjadi dipinjam lagi) -----
    if ($action == 'revertTransactionItem' && $method == 'POST') {
        if (!$isAdmin && !$isOperator) { http_response_code(403); echo json_encode(['error' => 'Forbidden']); exit; }
        ...
        try {
            ...
            $ti = $stmt->get_result()->fetch_assoc();
            if (!$ti) throw new Exception('Item transaksi tidak ditemukan');
            if ($ti['status'] != 'returned') throw new Exception('Item belum dikembalikan');

            // Update transaction_items: set status = 'borrowed', return_date = NULL, return_signature = NULL, return_photo = NULL
            $upd = $conn->prepare("UPDATE transaction_items 
                                   SET status = 'borrowed', return_date = NULL, return_signature = NULL, return_photo = NULL
                                   WHERE id = ?");
            $upd->bind_param('i', $transaction_item_id);
            $upd->execute();

            $upd2 = $conn->prepare("UPDATE items SET status = 'borrowed' WHERE id = ?");
            $upd2->bind_param('i', $ti['item_id']);
            $upd2->execute();

            // Update status transaksi: pasti menjadi 'borrowed' karena setidaknya ada satu item yang dipinjam
            $updTrans = $conn->prepare("UPDATE transactions SET status = 'borrowed' WHERE id = ?");
            $updTrans->bind_param('i', $ti['transaction_id']);
            $updTrans->execute();
    ...
    // ----- UPDATE TRANSACTION ITEM (edit field langsung) -----
    if ($action == 'updateTransactionItem' && $method == 'POST') {
        if (!$isAdmin && !$isOperator) { http_response_code(403); echo json_encode(['error' => 'Forbidden']); exit; }
        
        $input = getInput();
        $transaction_item_id = $input['id'];
        $borrow_date = $input['borrow_date'] ?? null;
        $return_date = $input['return_date'] ?? null;
        $status = $input['status'] ?? null;
        $return_signature = $input['return_signature'] ?? null;
        $return_photo = $input['return_photo'] ?? null; // asumsikan base64 atau path, perlu penanganan upload
        
        // Jika return_photo adalah file upload, perlu diproses secara terpisah. Untuk saat ini kita asumsikan string.
        
        $conn->begin_transaction();
        try {
            // Ambil data transaction_item saat ini
            $stmt = $conn->prepare("SELECT ti.*, i.id as item_id FROM transaction_items ti JOIN items i ON ti.item_id = i.id WHERE ti.id = ?");
            $stmt->bind_param('i', $transaction_item_id);
            $stmt->execute();
            $ti = $stmt->get_result()->fetch_assoc();
            ...
            // Validasi: jika status diubah, pastikan konsisten dengan item status
            $new_status = $status ?? $ti['status'];
            $new_return_date = $return_date ?? $ti['return_date'];
            $new_borrow_date = $borrow_date ?? $ti['borrow_date'];
            $new_return_signature = $return_signature ?? $ti['return_signature'];
            $new_return_photo = $return_photo ?? $ti['return_photo'];

            // Jika status berubah dari borrowed ke returned, set return_date jika belum ada
            if ($ti['status'] == 'borrowed' && $new_status == 'returned') {
                if (empty($new_return_date)) {
                    $new_return_date = date('Y-m-d H:i:s');
                }
                // Update item status menjadi available
                $updItem = $conn->prepare("UPDATE items SET status = 'available' WHERE id = ?");
                $updItem->bind_param('i', $ti['item_id']);
                $updItem->execute();
            }
            // Jika status berubah dari returned ke borrowed, hapus return_date dan set item status borrowed
            else if ($ti['status'] == 'returned' && $new_status == 'borrowed') {
                $new_return_date = null;
                $new_return_signature = null;
                $new_return_photo = null;
                $updItem = $conn->prepare("UPDATE items SET status = 'borrowed' WHERE id = ?");
                $updItem->bind_param('i', $ti['item_id']);
                $updItem->execute();
            }
            // Jika status tidak berubah, tetapi return_date diubah, pastikan item status sesuai
            else {
                // Jika return_date diisi (tidak null) dan status masih borrowed, kita perlu ubah status jadi returned?
                // Atau biarkan saja, tapi kita harus konsisten. Kita akan mengikuti status yang diberikan.
                if ($new_status == 'returned' && empty($new_return_date)) {
                    $new_return_date = date('Y-m-d H:i:s');
                }
                // Update item status sesuai new_status
                $item_status = ($new_status == 'borrowed') ? 'borrowed' : 'available';
                $updItem = $conn->prepare("UPDATE items SET status = ? WHERE id = ?");
                $updItem->bind_param('si', $item_status, $ti['item_id']);
                $updItem->execute();
            }

            // Update transaction_items
            $upd = $conn->prepare("UPDATE transaction_items 
                                   SET borrow_date = ?, return_date = ?, status = ?, return_signature = ?, return_photo = ? 
                                   WHERE id = ?");
            $upd->bind_param('sssssi', $new_borrow_date, $new_return_date, $new_status, $new_return_signature, $new_return_photo, $transaction_item_id);
            $upd->execute();

            // Periksa apakah semua item dalam transaksi sudah returned, update status transaksi
            $stmt2 = $conn->prepare("SELECT COUNT(*) AS cnt FROM transaction_items WHERE transaction_id = ? AND status = 'borrowed'");
            $stmt2->bind_param('i', $ti['transaction_id']);
            $stmt2->execute();
            $cnt_borrowed = $stmt2->get_result()->fetch_assoc()['cnt'];
            $trans_status = ($cnt_borrowed == 0) ? 'returned' : 'borrowed';
            $updTrans = $conn->prepare("UPDATE transactions SET status = ? WHERE id = ?");
            $updTrans->bind_param('si', $trans_status, $ti['transaction_id']);
            $updTrans->execute();
    ...
    // ----- DELETE TRANSACTION ITEM (hapus item dari transaksi, kembalikan status item ke available) -----
    if ($action == 'deleteTransactionItem' && $method == 'POST') {
        if (!$isAdmin && !$isOperator) { http_response_code(403); echo json_encode(['error' => 'Forbidden']); exit; }
        ...
        try {
            ...
            // Jika item masih borrowed, kembalikan status item menjadi available
            if ($ti['status'] == 'borrowed') {
                $upd = $conn->prepare("UPDATE items SET status = 'available' WHERE id = ?");
                $upd->bind_param('i', $ti['item_id']);
                $upd->execute();
            } else {
                // Jika sudah returned, status item sudah available, tidak perlu diubah
            }

            // Hapus dari transaction_items
            $del = $conn->prepare("DELETE FROM transaction_items WHERE id = ?");
            $del->bind_param('i', $transaction_item_id);
            $del->execute();

            // Periksa apakah masih ada item dalam transaksi
            $stmt2 = $conn->prepare("SELECT COUNT(*) AS cnt FROM transaction_items WHERE transaction_id = ?");
            $stmt2->bind_param('i', $ti['transaction_id']);
            $stmt2->execute();
            $cnt = $stmt2->get_result()->fetch_assoc()['cnt'];
            if ($cnt == 0) {
                // Jika tidak ada item lagi, hapus transaksi
                $delTrans = $conn->prepare("DELETE FROM transactions WHERE id = ?");
                $delTrans->bind_param('i', $ti['transaction_id']);
                $delTrans->execute();
            } else {
                // Update status transaksi jika semua item sudah returned
                $stmt3 = $conn->prepare("SELECT COUNT(*) AS cnt FROM transaction_items WHERE transaction_id = ? AND status = 'borrowed'");
                $stmt3->bind_param('i', $ti['transaction_id']);
                $stmt3->execute();
                $cnt_borrowed = $stmt3->get_result()->fetch_assoc()['cnt'];
                $new_status = ($cnt_borrowed == 0) ? 'returned' : 'borrowed';
                $updStatus = $conn->prepare("UPDATE transactions SET status = ? WHERE id = ?");
                $updStatus->bind_param('si', $new_status, $ti['transaction_id']);
                $updStatus->execute();
            }
    ...
    // ----- DASHBOARD STATS (MODIFIED: tambah total_returned untuk guru) -----
    if ($action == 'dashboard' && $method == 'GET') {
        ...
        if ($role == 'admin' || $role == 'operator') {
            ...
            // Total pengembalian (semua item yang sudah dikembalikan)
            $stmt3 = $conn->prepare("SELECT COUNT(*) as total FROM transaction_items ti JOIN transactions t ON ti.transaction_id = t.id WHERE t.user_id = ? AND ti.status = 'returned'");
            $stmt3->bind_param('i', $userId);
            $stmt3->execute();
            $stats['total_returned'] = $stmt3->get_result()->fetch_assoc()['total'];
        }
    ...
    // ========== GET TRANSACTION DETAIL (DIPERBAIKI UNTUK GURU) ==========
    if ($action == 'getTransactionDetail' && $method == 'GET') {
        $transId = $_GET['id'] ?? 0;
        // Pertama, cek apakah transaksi ada dan milik siapa
        $stmt = $conn->prepare("SELECT user_id FROM transactions WHERE id = ?");
        ...
        // Izinkan jika admin/operator, atau jika transaksi milik user yang sedang login (guru)
        if (!$isAdmin && !$isOperator && $trans['user_id'] != $_SESSION['user_id']) {
    ...
    if ($action == 'updateTransaction' && $method == 'POST') {
        ...
        $new_borrow_signature = $input['borrow_signature'] ?? null;
        $new_borrow_photo = $input['borrow_photo'] ?? null; // asumsikan string path
        $new_return_signature = $input['return_signature'] ?? null; // mungkin untuk transaksi level
        ...
        try {
            ...
            // Update data transaksi (user_id, room_id, transaction_date, notes, signatures, photos)
            if (empty($new_transaction_date)) {
                $upd_trans = $conn->prepare("UPDATE transactions SET user_id = ?, room_id = ?, notes = ?, borrow_signature = ?, borrow_photo = ?, return_signature = ? WHERE id = ?");
                $upd_trans->bind_param('iissssi', $new_user_id, $new_room_id, $new_notes, $new_borrow_signature, $new_borrow_photo, $new_return_signature, $trans_id);
            } else {
                $upd_trans = $conn->prepare("UPDATE transactions SET user_id = ?, room_id = ?, transaction_date = ?, notes = ?, borrow_signature = ?, borrow_photo = ?, return_signature = ? WHERE id = ?");
                $upd_trans->bind_param('iississsi', $new_user_id, $new_room_id, $new_transaction_date, $new_notes, $new_borrow_signature, $new_borrow_photo, $new_return_signature, $trans_id);
            }
            $upd_trans->execute();
    ...
    // ----- GET REPORT (MODIFIED: tambahkan borrow_code, ruang, dan status grup, serta signature data) -----
    if ($action == 'getReport' && $method == 'GET') {
        ...
        $data = [];
        while ($row = $result->fetch_assoc()) {
            // Simpan data signature mentah (mungkin base64) untuk ditampilkan nanti
            $data[] = [
                'borrow_code' => $row['borrow_code'] ?? '-',
                ...
                'item_code' => $row['item_code'],
                'item_name' => $row['item_name'],
                'borrow_signature' => $row['borrow_signature'],
                'return_signature' => $row['return_signature'],
                'borrow_photo' => $row['borrow_photo'],
                'return_photo' => $row['return_photo'],
                'keterangan' => $row['item_status'] == 'returned' ? 'Dikembalikan' : 'Dipinjam'
            ];
        }

        // Kelompokkan berdasarkan borrow_code
        $grouped = [];
        foreach ($data as $row) {
            $code = $row['borrow_code'];
        ...
        // Hitung status grup untuk setiap transaksi
        foreach ($grouped as &$group) {
            $totalItems = count($group['items']);
            $returnedItems = 0;
            foreach ($group['items'] as $item) {
                if ($item['keterangan'] == 'Dikembalikan') {
                    $returnedItems++;
                }
            }
            if ($returnedItems == 0) {
                $group['group_status'] = 'Dipinjam';
                $group['group_status_class'] = 'bg-yellow-100 text-yellow-800';
            } elseif ($returnedItems == $totalItems) {
                $group['group_status'] = 'Dikembalikan';
                $group['group_status_class'] = 'bg-green-100 text-green-800';
            } else {
                $group['group_status'] = 'Sebagian Dikembalikan';
                $group['group_status_class'] = 'bg-gray-100 text-gray-800';
            }
        }

        // Ubah ke array biasa
        $resultData = array_values($grouped);
        echo json_encode(['data' => $resultData, 'month' => $month, 'year' => $year]);
        exit;
    }

    // ========== GET REKAPITULASI ==========
    if ($action == 'getRekap' && $method == 'GET') {
        if (!$isAdmin && !$isOperator) { http_response_code(403); echo json_encode(['error' => 'Forbidden']); exit; }
        
        $periode = $_GET['periode'] ?? 'harian'; // harian, mingguan, bulanan, tahunan
        
        // Tentukan interval query berdasarkan periode
        switch ($periode) {
            case 'harian':
            ...
            case 'mingguan':
                ...
                $dateFormat = "%Y-%u"; // tahun-minggu
                $labelFormat = "'Minggu ke-' W 'Tahun' Y";
                break;
            case 'bulanan':
        ...
        $sql = "
            SELECT 
                ...
                SUM(CASE WHEN t.status = 'borrowed' THEN 1 ELSE 0 END) as aktif,
                SUM(CASE WHEN t.status = 'returned' THEN 1 ELSE 0 END) as selesai,
                COUNT(ti.id) as total_item_dipinjam,
                SUM(CASE WHEN ti.status = 'returned' THEN 1 ELSE 0 END) as total_item_kembali
            FROM transactions t
            LEFT JOIN transaction_items ti ON t.id = ti.transaction_id
            GROUP BY $groupBy
        ...
        $peminjam = $conn->query("
            SELECT u.name, COUNT(t.id) as jumlah
            FROM transactions t
            JOIN users u ON t.user_id = u.id
            WHERE u.role = 'guru'
            GROUP BY t.user_id
        ...
        // Statistik peralatan paling sering dipinjam
        $peralatan = $conn->query("
            SELECT i.name, COUNT(ti.id) as jumlah
            FROM transaction_items ti
            JOIN items i ON ti.item_id = i.id
            GROUP BY ti.item_id
            ORDER BY jumlah DESC
        ...
        // Statistik ruang paling sering digunakan
        $ruang = $conn->query("
            SELECT r.name, COUNT(t.id) as jumlah
            FROM transactions t
            JOIN rooms r ON t.room_id = r.id
            GROUP BY t.room_id
            ORDER BY jumlah DESC
        ...
        // Statistik tambahan
        $totalUser = $conn->query("SELECT COUNT(*) as total FROM users WHERE role='guru'")->fetch_assoc()['total'];
        $totalItem = $conn->query("SELECT COUNT(*) as total FROM items")->fetch_assoc()['total'];
        $totalTransaksi = $conn->query("SELECT COUNT(*) as total FROM transactions")->fetch_assoc()['total'];
        $totalPinjamAktif = $conn->query("SELECT COUNT(*) as total FROM transaction_items WHERE status='borrowed'")->fetch_assoc()['total'];
...
<head>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .content-with-bottom-nav { padding-bottom: 8rem; }
        ...
        .tab-button-monthly-active {
            @apply bg-blue-600 text-white shadow-md border-blue-600;
        }
        .tab-button-bycode-active {
            @apply bg-green-600 text-white shadow-md border-green-600;
        }
        .tab-button-rekap-active {
            @apply bg-purple-600 text-white shadow-md border-purple-600;
        }
        .tab-button-inactive {
            @apply bg-gray-100 text-gray-700 hover:bg-gray-200 border-gray-200;
        }
...
<body class="bg-gray-100">
    ...
    <div class="min-h-screen flex items-center justify-center">
        <div class="bg-white p-8 rounded-lg shadow-md w-96">
            <div class="text-center mb-6">
                <i class="fas fa-dolly text-5xl text-blue-600"></i>
                <h1 class="text-2xl font-bold mt-2">Amanat Sekolah V1.0a</h1> [Aplikasi Peminjaman Peralatan Sekolah]</h1>
    ...
    <div id="app" class="min-h-screen flex flex-col" v-cloak>
        ...
        <nav class="bg-blue-600 text-white shadow-lg">
            <div class="container mx-auto px-4 py-3 flex justify-between items-center">
                <h1 class="text-xl font-bold flex items-center gap-2"><i class="fas fa-qrcode"></i> <span>Amanat v1a</span></h1>
        ...
        <div v-if="showEditTransactionDetailModal" class="modal fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4" @click.self="showEditTransactionDetailModal = false">
            <div class="bg-white rounded-lg max-w-3xl w-full p-6">
                ...
                <div class="space-y-4">
                    ...
                    <!-- Bagian bukti peminjaman -->
                    <div class="border-t pt-4 mt-2">
                        <h4 class="font-semibold mb-2">Bukti Peminjaman</h4>
                        <div class="grid grid-cols-2 gap-4">
                            <div>
                                <label class="block text-sm font-medium text-gray-700 mb-1">Tanda Tangan Peminjam</label>
                                <div v-if="editTransactionData.borrow_signature && editTransactionData.borrow_signature !== 'dummy'" class="mb-2">
                                    <img :src="editTransactionData.borrow_signature" class="signature-img border" />
                                </div>
                                <button @click="captureBorrowSignature" class="bg-blue-600 text-white px-3 py-1 rounded text-sm">Ambil Tanda Tangan Baru</button>
                            </div>
                            <div>
                                <label class="block text-sm font-medium text-gray-700 mb-1">Foto Peminjaman</label>
                                <div v-if="editTransactionData.borrow_photo" class="mb-2">
                                    <img :src="editTransactionData.borrow_photo" class="max-w-full h-20 object-cover border" />
                                </div>
                                <input type="file" @change="onBorrowPhotoChange" accept="image/*" class="border rounded px-3 py-1 w-full text-sm">
                            </div>
                        </div>
                    </div>
                    ...
                    <div>
                        ...
                        <div class="space-y-2 max-h-60 overflow-y-auto border p-2 rounded">
                            <div v-for="item in editTransactionData.items" :key="item.item_id" class="flex justify-between items-center bg-gray-50 p-2 rounded">
                                ...
                                <div>
                                    <button @click="openEditTransactionItem(item)" class="text-blue-600 hover:text-blue-800 mr-2" title="Edit Item"><i class="fas fa-edit"></i></button>
        ...
        <!-- Modal Edit Item Transaksi (untuk edit per item) -->
        <div v-if="showEditTransactionItemModal" class="modal fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4" @click.self="showEditTransactionItemModal = false">
            <div class="bg-white rounded-lg max-w-lg w-full p-6">
                <h3 class="font-bold text-lg mb-4">Edit Item Transaksi</h3>
                <div class="space-y-3">
                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-1">Tanggal Pinjam</label>
                        <input type="datetime-local" v-model="editTransactionItemData.borrow_date" class="border rounded px-3 py-2 w-full">
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-1">Tanggal Kembali</label>
                        <input type="datetime-local" v-model="editTransactionItemData.return_date" class="border rounded px-3 py-2 w-full">
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-1">Status</label>
                        <select v-model="editTransactionItemData.status" class="border rounded px-3 py-2 w-full">
                            <option value="borrowed">Dipinjam</option>
                            <option value="returned">Dikembalikan</option>
                        </select>
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-1">Tanda Tangan Pengembalian</label>
                        <div v-if="editTransactionItemData.return_signature" class="mb-2">
                            <img :src="editTransactionItemData.return_signature" class="signature-img border" />
                        </div>
                        <button @click="captureReturnSignature" class="bg-blue-600 text-white px-3 py-1 rounded text-sm">Ambil Tanda Tangan Baru</button>
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-1">Foto Pengembalian</label>
                        <div v-if="editTransactionItemData.return_photo" class="mb-2">
                            <img :src="editTransactionItemData.return_photo" class="max-w-full h-20 object-cover border" />
                        </div>
                        <input type="file" @change="onItemReturnPhotoChange" accept="image/*" class="border rounded px-3 py-1 w-full text-sm">
                    </div>
                </div>
                <div class="flex justify-end gap-2 mt-4">
                    <button @click="showEditTransactionItemModal = false" class="px-4 py-2 bg-gray-200 rounded">Batal</button>
                    <button @click="saveTransactionItem" class="px-4 py-2 bg-blue-600 text-white rounded hover:bg-blue-700">Simpan</button>
                </div>
            </div>
        </div>
    </div>
    ...
    <script>
        const { createApp, ref, computed, onMounted, markRaw, watch, onUnmounted } = Vue;

        const App = {
            setup() {
                // ---------- Fungsi format tanggal Indonesia ----------
                const formatIndonesianDate = (dateStr, includeTime = false) => {
                    if (!dateStr) return '-';
                ...
                // Fungsi untuk mengubah string tanggal ke format datetime-local
                ...
                const itemReturnPhotoFile = ref(null); // untuk item edit

                // State untuk form tambah item
                const newItem = ref({ category_id: '', brand_id: '', type_id: '', name: '', year: '' });

                // State untuk modal edit transaksi detail (header)
                ...
                // State untuk modal edit item transaksi
                const showEditTransactionItemModal = ref(false);
                const editTransactionItemData = ref({
                    id: null,
                    borrow_date: '',
                    return_date: '',
                    status: 'borrowed',
                ...
                // State untuk input manual kode pada peminjaman dan pengembalian
                const manualBorrowCode = ref('');
                const manualReturnCode = ref('');
                const manualBorrowMessage = ref('');
                const manualReturnMessage = ref('');
                ...
                // ---------- FUNGSI UNTUK EDIT TRANSAKSI DETAIL (HEADER) ----------
                ...
                async function saveTransactionDetail() {
                    if (!editTransactionData.value.user_id || !editTransactionData.value.room_id) {
                    ...
                    const payload = {
                        ...
                        transaction_date: editTransactionData.value.transaction_date || null,
                        items: editTransactionData.value.items.map(item => item.item_id),
                        notes: editTransactionData.value.notes,
                        borrow_signature: editTransactionData.value.borrow_signature,
                        borrow_photo: editTransactionData.value.borrow_photo,
                        return_signature: editTransactionData.value.return_signature
                    };
                ...
                async function captureBorrowSignature() {
                    const signature = await openSignatureModal('Tanda Tangan Peminjam');
                    if (signature) {
                        editTransactionData.value.borrow_signature = signature;
                    }
                }

                function onBorrowPhotoChange(e) {
                    const file = e.target.files[0];
                    if (file) {
                        const reader = new FileReader();
                        reader.onload = (event) => {
                            // Simpan sebagai base64 sementara, nanti bisa diupload via form data
                            // Untuk sementara kita simpan sebagai data URL, tapi perlu dikirim ke server
                            // Server akan menyimpan path file, bukan base64. Jadi perlu upload.
                            // Tapi untuk kemudahan, kita bisa upload file dulu.
                            // Untuk sekarang, kita simpan file object dan upload nanti.
                            // Kita gunakan FormData di saveTransactionDetail? Tapi payload JSON.
                            // Alternatif: kita upload file terpisah dan dapatkan path, lalu set.
                            // Sederhananya, kita set dulu sebagai base64 sementara.
                            editTransactionData.value.borrow_photo = event.target.result;
                        };
                ...
                // ---------- FUNGSI UNTUK RETURN ITEM INDIVIDUAL ----------
                async function returnTransactionItem(transactionItemId) {
                    ...
                    const res = await fetch('?action=returnTransactionItem', {
                        method: 'POST',
                        headers: { 'Content-Type': 'application/json' },
                        body: JSON.stringify({ transaction_item_id: transactionItemId, return_signature: returnSignature })
                    });
                    const result = await res.json();
                    if (result.success) {
                ...
                // ---------- FUNGSI UNTUK REVERT ITEM INDIVIDUAL (mengembalikan status menjadi dipinjam) ----------
                async function revertTransactionItem(transactionItemId) {
                    if (!confirm('Batalkan pengembalian item ini? Item akan kembali ke status dipinjam.')) return;
                    
                    const res = await fetch('?action=revertTransactionItem', {
                        method: 'POST',
                        headers: { 'Content-Type': 'application/json' },
                        body: JSON.stringify({ transaction_item_id: transactionItemId })
                    });
                    const result = await res.json();
                    if (result.success) {
                        alert('Item berhasil dikembalikan ke status dipinjam');
                        if (refreshTransactions) refreshTransactions();
                    } else {
                        alert('Gagal: ' + result.error);
                    }
                ...
                // ---------- FUNGSI UNTUK EDIT ITEM INDIVIDUAL ----------
                function openEditTransactionItem(item) {
                    editTransactionItemData.value = {
                        id: item.id,
                        borrow_date: toDatetimeLocal(item.borrow_date),
                        return_date: toDatetimeLocal(item.return_date),
                        status: item.status,
                        return_signature: item.return_signature,
                        return_photo: item.return_photo
                    };
                    itemReturnPhotoFile.value = null;
                    showEditTransactionItemModal.value = true;
                }

                async function captureReturnSignature() {
                    const signature = await openSignatureModal('Tanda Tangan Pengembalian');
                    if (signature) {
                        editTransactionItemData.value.return_signature = signature;
                    }
                }

                function onItemReturnPhotoChange(e) {
                    const file = e.target.files[0];
                    if (file) {
                        const reader = new FileReader();
                        reader.onload = (event) => {
                            editTransactionItemData.value.return_photo = event.target.result;
                        };
                        reader.readAsDataURL(file);
                    }
                }

                async function saveTransactionItem() {
                    const payload = {
                        id: editTransactionItemData.value.id,
                        borrow_date: editTransactionItemData.value.borrow_date || null,
                        return_date: editTransactionItemData.value.return_date || null,
                        status: editTransactionItemData.value.status,
                        return_signature: editTransactionItemData.value.return_signature,
                        return_photo: editTransactionItemData.value.return_photo
                    };
                    const res = await fetch('?action=updateTransactionItem', {
                        method: 'POST',
                        headers: { 'Content-Type': 'application/json' },
                        body: JSON.stringify(payload)
                    });
                    ...
                    if (result.success) {
                        ...
                        showEditTransactionItemModal.value = false;
                        if (refreshTransactions) refreshTransactions();
                    } else {
                        alert('Gagal: ' + result.error);
                    }
                ...
                async function deleteTransactionItem(transactionItemId) {
                    if (!confirm('Yakin ingin menghapus item ini dari transaksi? Status item akan dikembalikan menjadi tersedia.')) return;
                    
                    const res = await fetch('?action=deleteTransactionItem', {
                        method: 'POST',
                        headers: { 'Content-Type': 'application/json' },
                        body: JSON.stringify({ transaction_item_id: transactionItemId })
                    });
                    const result = await res.json();
                    if (result.success) {
                        alert('Item berhasil dihapus dari transaksi');
                        if (refreshTransactions) refreshTransactions();
                    } else {
                        alert('Gagal: ' + result.error);
                    }
                ...
                // ---------- FUNGSI INPUT MANUAL UNTUK PEMINJAMAN ----------
                async function addManualBorrowItem() {
                    if (!manualBorrowCode.value) return alert('Masukkan kode peralatan');
                    const res = await fetch(`?action=getItemByCode&code=${encodeURIComponent(manualBorrowCode.value)}`);
                    const item = await res.json();
                    if (!item) {
                        manualBorrowMessage.value = 'Item tidak ditemukan';
                        return;
                    }
                    if (item.status !== 'available') {
                        manualBorrowMessage.value = `Item sedang ${item.status}`;
                        return;
                    }
                    if (cart.value.find(i => i.id === item.id)) {
                        manualBorrowMessage.value = 'Item sudah ada di keranjang';
                        return;
                    }
                    addToCart(item);
                    manualBorrowMessage.value = 'Item ditambahkan ke keranjang';
                    manualBorrowCode.value = '';
                }

                // ---------- FUNGSI INPUT MANUAL UNTUK PENGEMBALIAN ----------
                async function processManualReturn() {
                    if (!manualReturnCode.value) return alert('Masukkan kode peralatan');
                    const res = await fetch(`?action=getItemByCode&code=${encodeURIComponent(manualReturnCode.value)}`);
                    const item = await res.json();
                    if (!item) {
                        manualReturnMessage.value = 'Item tidak ditemukan';
                        return;
                    }
                    if (item.status !== 'borrowed') {
                        manualReturnMessage.value = `Item tidak sedang dipinjam (status: ${item.status})`;
                        return;
                    }
                    ...
                    if (result.success) {
                        manualReturnMessage.value = `Item ${item.name} berhasil dikembalikan.`;
                        returnPhotoFile.value = null;
                        manualReturnCode.value = '';
                    } else {
                        manualReturnMessage.value = 'Error: ' + result.error;
                    }
                }
                ...
                onMounted(async () => {
                    ...
                    await loadCategories(); // Semua user bisa lihat kategori
                ...
                Vue.provide('app', {
                    ...
                    openAddRoomModal, openAddGuruModal,
                    ...
                    // Provide fungsi item individual
                    returnTransactionItem, revertTransactionItem, deleteTransactionItem, openEditTransactionItem, saveTransactionItem,
                    // Provide fungsi manual
                    manualBorrowCode, manualReturnCode, manualBorrowMessage, manualReturnMessage,
                    addManualBorrowItem, processManualReturn,
                    // Provide fungsi format tanggal
                ...
                return {
                    ...
                    captureBorrowSignature, onBorrowPhotoChange,
                    // Expose modal edit item transaksi
                    showEditTransactionItemModal, editTransactionItemData, saveTransactionItem,
                    captureReturnSignature, onItemReturnPhotoChange,
        ...
        // =============== DASHBOARD (MODIFIED: ditambahkan konten Rekapitulasi dengan grafik responsif) ===============
        const Dashboard = {
            template: `
                <div>
                    ...
                    <div v-else>
                        <!-- Statistik 4 kolom untuk Admin/Operator -->
                        <div v-if="isAdmin || isOperator" class="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
                            <div class="bg-blue-50 p-4 rounded-lg border border-blue-200 text-center">
                                <div class="text-blue-700 text-sm">Total Item</div>
                                <div class="text-2xl font-bold">{{ stats.total_items || 0 }}</div>
                            </div>
                            <div class="bg-green-50 p-4 rounded-lg border border-green-200 text-center">
                                <div class="text-green-700 text-sm">Tersedia</div>
                                <div class="text-2xl font-bold">{{ stats.available_items || 0 }}</div>
                            </div>
                            <div class="bg-yellow-50 p-4 rounded-lg border border-yellow-200 text-center">
                                <div class="text-yellow-700 text-sm">Dipinjam</div>
                                <div class="text-2xl font-bold">{{ stats.borrowed_items || 0 }}</div>
                            </div>
                            <div class="bg-purple-50 p-4 rounded-lg border border-purple-200 text-center">
                                <div class="text-purple-700 text-sm">Transaksi Hari Ini</div>
                                <div class="text-2xl font-bold">{{ stats.today_transactions || 0 }}</div>
                            </div>
                        </div>

                        <!-- [ADD] Bagian Rekapitulasi untuk Admin/Operator dengan grafik responsif -->
                        <div v-if="isAdmin || isOperator" class="mt-8 border-t pt-6">
                            <div class="mb-4">
                                <label class="block text-sm font-medium text-gray-700 mb-1">Periode Rekapitulasi</label>
                                <select v-model="rekapPeriode" @change="fetchRekap" class="border rounded px-3 py-2">
                            ...
                            <!-- Statistik Angka Rekap -->
                            <div class="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
                                <div class="bg-blue-50 p-4 rounded-lg border border-blue-200 text-center">
                                    <div class="text-blue-700 text-sm">Total Guru</div>
                                    <div class="text-2xl font-bold">{{ rekapStats.totalUser || 0 }}</div>
                                </div>
                                <div class="bg-green-50 p-4 rounded-lg border border-green-200 text-center">
                                    <div class="text-green-700 text-sm">Total Item</div>
                                    <div class="text-2xl font-bold">{{ rekapStats.totalItem || 0 }}</div>
                                </div>
                                <div class="bg-yellow-50 p-4 rounded-lg border border-yellow-200 text-center">
                                    <div class="text-yellow-700 text-sm">Total Transaksi</div>
                                    <div class="text-2xl font-bold">{{ rekapStats.totalTransaksi || 0 }}</div>
                                </div>
                                <div class="bg-purple-50 p-4 rounded-lg border border-purple-200 text-center">
                                    <div class="text-purple-700 text-sm">Pinjam Aktif</div>
                                    <div class="text-2xl font-bold">{{ rekapStats.totalPinjamAktif || 0 }}</div>
                                </div>
                            </div>

                            <!-- Grafik dengan wrapper responsif -->
                            <div v-if="rekapLoading" class="text-center py-5"><i class="fas fa-spinner fa-spin"></i></div>
                            <div v-else>
                                <div class="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-8">
                                    <div class="bg-white p-4 rounded-lg shadow">
                                        <h3 class="font-semibold mb-3">Tren Peminjaman</h3>
                                        <div class="relative h-64">
                                            <canvas id="dashboardChartTransaksi"></canvas>
                                        </div>
                                    </div>
                                    <div class="bg-white p-4 rounded-lg shadow">
                                        <h3 class="font-semibold mb-3">Top Peminjam</h3>
                                        <div class="relative h-64">
                                            <canvas id="dashboardChartPeminjam"></canvas>
                                        </div>
                                    </div>
                                </div>
                                ...
                                <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
                                    <div class="bg-white p-4 rounded-lg shadow">
                                        <h3 class="font-semibold mb-3">Peralatan Terpopuler</h3>
                                        <div class="relative h-64">
                                            <canvas id="dashboardChartPeralatan"></canvas>
                                        </div>
                                    </div>
                                    <div class="bg-white p-4 rounded-lg shadow">
                                        <h3 class="font-semibold mb-3">Ruang Terpadat</h3>
                                        <div class="relative h-64">
                                            <canvas id="dashboardChartRuang"></canvas>
                                        </div>
                                    </div>
                                </div>
                            </div>
                        ...
                        <div v-if="!isAdmin && !isOperator" class="grid grid-cols-2 gap-4 mb-8">
                            <div class="bg-yellow-50 p-4 rounded-lg border border-yellow-200 text-center">
                                <div class="text-yellow-700 text-sm">Sedang Dipinjam</div>
                                <div class="text-2xl font-bold">{{ stats.my_borrowed_items?.length || 0 }}</div>
                            </div>
                            <div class="bg-green-50 p-4 rounded-lg border border-green-200 text-center">
                                <div class="text-green-700 text-sm">Sudah Dikembalikan</div>
                                <div class="text-2xl font-bold">{{ stats.total_returned || 0 }}</div>
                            </div>
                        </div>
                ...
                </div><div style="margin-bottom:70px;"></div>
            `,
            setup() {
                const app = Vue.inject('app');
                const isAdmin = Vue.computed(() => app.isAdmin.value);
                const isOperator = Vue.computed(() => app.isOperator.value);
                const formatIndonesianDate = app.formatIndonesianDate;
                const stats = Vue.ref({});
                const loading = Vue.ref(true);

                // Rekap state
                const rekapPeriode = Vue.ref('bulanan');
                const rekapStats = Vue.ref({});
                const rekapLoading = Vue.ref(false);
                let chartTransaksi, chartPeminjam, chartPeralatan, chartRuang;
                ...
                async function fetchRekap() {
                    ...
                    const res = await fetch(`?action=getRekap&periode=${rekapPeriode.value}`);
                    const data = await res.json();
                    rekapStats.value = data;
                ...
                function renderCharts(data) {
                    ...
                    if (chartPeminjam) chartPeminjam.destroy();
                    if (chartPeralatan) chartPeralatan.destroy();
                    if (chartRuang) chartRuang.destroy();

                    // Chart tren transaksi
                    const ctxTransaksi = document.getElementById('dashboardChartTransaksi')?.getContext('2d');
                    if (ctxTransaksi && data.chart) {
                        chartTransaksi = new Chart(ctxTransaksi, {
                            type: 'line',
                            data: {
                                labels: data.chart.map(d => d.periode),
                                datasets: [
                                    { label: 'Total Transaksi', data: data.chart.map(d => d.total_transaksi), borderColor: 'blue', fill: false },
                                    { label: 'Item Dipinjam', data: data.chart.map(d => d.total_item_dipinjam), borderColor: 'green', fill: false }
                                ]
                            },
                            options: {
                                responsive: true,
                    ...
                    // Chart top peminjam
                    const ctxPeminjam = document.getElementById('dashboardChartPeminjam')?.getContext('2d');
                    if (ctxPeminjam && data.peminjam) {
                        chartPeminjam = new Chart(ctxPeminjam, {
                            type: 'bar',
                            data: {
                                labels: data.peminjam.map(p => p.name),
                                datasets: [{ label: 'Jumlah Pinjam', data: data.peminjam.map(p => p.jumlah), backgroundColor: 'orange' }]
                            },
                            options: {
                                responsive: true,
                                maintainAspectRatio: false
                    ...
                    // Chart peralatan
                    const ctxPeralatan = document.getElementById('dashboardChartPeralatan')?.getContext('2d');
                    if (ctxPeralatan && data.peralatan) {
                        chartPeralatan = new Chart(ctxPeralatan, {
                            type: 'bar',
                            data: {
                                labels: data.peralatan.map(p => p.name),
                                datasets: [{ label: 'Jumlah Dipinjam', data: data.peralatan.map(p => p.jumlah), backgroundColor: 'teal' }]
                            },
                            options: {
                                responsive: true,
                                maintainAspectRatio: false
                    ...
                    // Chart ruang
                    const ctxRuang = document.getElementById('dashboardChartRuang')?.getContext('2d');
                    if (ctxRuang && data.ruang) {
                        chartRuang = new Chart(ctxRuang, {
                            type: 'bar',
                            data: {
                                labels: data.ruang.map(r => r.name),
                                datasets: [{ label: 'Jumlah Penggunaan', data: data.ruang.map(r => r.jumlah), backgroundColor: 'purple' }]
                            },
                            options: {
                                responsive: true,
                                maintainAspectRatio: false
                ...
                function formatDate(d) {
                    return formatIndonesianDate(d, false);
                }
                ...
                return { 
                    isAdmin, isOperator, stats, loading, formatDate, 
                    roleIcon: Vue.computed(() => isAdmin.value ? 'fas fa-user-cog' : (isOperator.value ? 'fas fa-user-tie' : 'fas fa-chalkboard-teacher')),
                    // Rekap
                    rekapPeriode, rekapStats, rekapLoading, fetchRekap
                };
            }
        };

        // =============== KATEGORI, MERK, TIPE (dengan style tab baru) ===============
        const CategoryBrand = {
            template: `
                <div>
                    <h2 class="text-2xl font-bold mb-5">Manajemen Kategori, Merk & Tipe</h2>
                    <div class="mb-4 flex flex-wrap gap-2">
                        <button @click="activeTab = 'categories'" :class="['tab-button', activeTab === 'categories' ? 'tab-button-monthly-active' : 'tab-button-inactive']">
                            Kategori
                        </button> > 
                        <button @click="activeTab = 'brands'" :class="['tab-button', activeTab === 'brands' ? 'tab-button-bycode-active' : 'tab-button-inactive']">
                            Merk
                        </button> >
                        <button @click="activeTab = 'types'" :class="['tab-button', activeTab === 'types' ? 'tab-button-rekap-active' : 'tab-button-inactive']">
                            Tipe
                        </button>
                    </div>
                    ...
                    <div v-if="activeTab === 'categories'">
                        <div class="mb-4 flex flex-wrap gap-2">
                            <input v-model="newCategory.name" placeholder="Nama Kategori" class="border rounded px-3 py-2 flex-1 min-w-[200px]">
                            <input v-model="newCategory.description" placeholder="Deskripsi" class="border rounded px-3 py-2 flex-1 min-w-[200px]">
                    ...
                    <div v-if="activeTab === 'brands'">
                        <div class="mb-4 flex flex-wrap gap-2">
                            ...
                            <input v-model="newBrand.name" placeholder="Nama Merk" class="border rounded px-3 py-2 flex-1 min-w-[200px]">
                    ...
                    <div v-if="activeTab === 'types'">
                        <div class="mb-4 flex flex-wrap gap-2">
                            ...
                            <input v-model="newType.name" placeholder="Nama Tipe" class="border rounded px-3 py-2 flex-1 min-w-[200px]">
            ...
            setup() {
                ...
                const categories = app.categories; // sudah reactive dari provide
        ...
        // =============== ITEM LIST (DIPERBAIKI UNTUK GURU, DITAMBAH FILTER TIPE DAN TOMBOL CETAK QR) ===============
        const ItemList = {
            template: `
                <div>
                    ...
                    <div class="flex flex-wrap gap-2 mb-4 items-center">
                        ...
                        <select v-model="selectedBrand" @change="onBrandChange" class="border rounded px-3 py-2 text-sm" :disabled="!selectedCategory && brandsFilter.length === 0">
                            <option value="">Semua Merk</option>
                            <option v-for="brand in brandsFilter" :key="brand.id" :value="brand.id">{{ brand.name }}</option>
                        </select>

                        <!-- [ADD] Filter Tipe (bergantung pada merk) -->
                        <select v-model="selectedType" @change="applyFilters" class="border rounded px-3 py-2 text-sm" :disabled="!selectedBrand && typesFilter.length === 0">
                            <option value="">Semua Tipe</option>
                            <option v-for="type in typesFilter" :key="type.id" :value="type.id">{{ type.name }}</option>
                        </select>
                        ...
                        <div class="relative flex-1 min-w-[200px]">
                            <input type="text" v-model="searchQuery" @input="debouncedSearch" placeholder="Cari nama, kode, kategori, merk, tipe..." class="border rounded px-3 py-2 w-full pr-8 text-sm">
                        ...
                        <!-- Tombol Tambah Item (untuk admin) dan Cetak QR (untuk admin/operator) -->
                        <div class="flex gap-2">
                            ...
                            <button v-if="isAdmin || isOperator" @click="printQRItems" class="bg-purple-600 text-white px-4 py-2 rounded hover:bg-purple-700 text-sm whitespace-nowrap">
                                <i class="fas fa-qrcode"></i> Cetak QR
                            </button>
                        </div>
                    </div>

                    <!-- [ADD] Info Jumlah Item Tersedia -->
                    <div v-if="!loading" class="mb-3 p-3 bg-blue-50 border border-blue-200 rounded-lg">
                        <div class="flex items-center justify-between">
                            <div>
                                <span class="font-medium text-blue-800">Item Tersedia:</span>
                                <span class="ml-2 text-lg font-bold text-blue-600">{{ availableCount }}</span>
                            </div>
                        ...
                        <div v-for="item in items" :key="item.id" class="border rounded-lg p-4 shadow-sm hover:shadow-md transition break-words">
                            <div class="flex justify-between items-start">
                                <div class="flex-1 min-w-0">
                                    ...
                                    <span :class="['badge', statusClass(item.status)]" class="mt-2">{{ item.status === 'available' ? 'Tersedia' : item.status === 'borrowed' ? 'Dipinjam' : item.status }}</span>
                                </div>
            ...
            setup() {
                ...
                const selectedType = ref(''); // [ADD] State untuk filter tipe
                const sortOrder = ref('asc');
                const searchQuery = ref('');
                const searchLoading = ref(false);
                const loading = ref(true);
                const brandsFilter = ref([]);
                const typesFilter = ref([]); // [ADD] State untuk daftar tipe berdasarkan merk yang dipilih

                // [ADD] Computed untuk menghitung jumlah item tersedia dari hasil filter saat ini
                const availableCount = computed(() => {
                    return items.value.filter(item => item.status === 'available').length;
                });

                // Watch untuk perubahan kategori: reset brand & type, lalu load brand filter
                watch(selectedCategory, async (newVal) => {
                    selectedBrand.value = '';
                    selectedType.value = ''; // Reset tipe juga
                    if (newVal) {
                ...
                // [ADD] Watch untuk perubahan merk: reset type, lalu load type filter
                watch(selectedBrand, async (newVal) => {
                    selectedType.value = ''; // Reset tipe
                    if (newVal) {
                        const res = await fetch(`?action=getTypesByBrand&brand_id=${newVal}`);
                        typesFilter.value = await res.json();
                    } else {
                        typesFilter.value = [];
                    }
                    applyFilters(); // Terapkan filter saat merk berubah
                });

                // [ADD] Fungsi untuk menangani perubahan merk (dari template)
                function onBrandChange() {
                    // Watcher sudah handle, jadi cukup kosong
                }

                function onCategoryChange() {
                    // Watcher sudah handle
                }
                ...
                async function fetchItems() {
                    ...
                    if (selectedType.value) url += `&type_id=${selectedType.value}`; // [ADD] Parameter filter tipe
                ...
                function statusClass(status) {
                    return { 'available': 'badge-success', 'borrowed': 'badge-warning', 'damaged': 'badge-danger', 'lost': 'badge-gray' }[status] || 'badge-gray';
                }
                ...
                // [ADD] Fungsi untuk mencetak QR Code dalam format A4 6 baris x 5 kolom
                async function printQRItems() {
                    const itemsToPrint = items.value;
                    if (itemsToPrint.length === 0) {
                        alert('Tidak ada item untuk dicetak');
                        return;
                    }

                    // Bagi per halaman (30 item per halaman)
                    ...
                    // Generate semua QR code sebagai data URL
                    const generatePromises = itemsToPrint.map(item => {
                        return new Promise((resolve) => {
                            QRCode.toDataURL(item.qr_code, { width: 150, margin: 1 }, (err, url) => {
                                if (err) {
                                    console.error(err);
                                    resolve(null);
                                } else {
                    ...
                    const qrMap = new Map();
                    results.forEach(r => {
                        if (r) qrMap.set(r.item.id, r.url);
                    });
                    ...
                    <html>
                    <head>
                        <title>Cetak QR Code Peralatan</title>
                        <style>
                            ...
                            .qr-item img { width: 100%; max-width: 120px; height: auto; margin: 0 auto; display: block; }
                            .qr-item .code { font-size: 12px; margin-top: 5px; word-break: break-word; }
                        </style>
                    </head>
                    <body>
                    `;
                        ...
                        pageItems.forEach(item => {
                            ...
                            html += `
                                <div class="qr-item">
                                    ...
                                    <div class="code" style="font-size:10px;">${item.name}</div>
                                </div>
                            `;
                        });
                        // Jika halaman kurang dari 30, grid akan tetap rapi karena CSS grid menyesuaikan
                        html += `</div>`;
                    }
                ...
                return { 
                    isAdmin, isOperator, items, categories, brands, types, loading, filterStatus, 
                    ...
                    selectedCategory, selectedBrand, selectedType, sortOrder, searchQuery, searchLoading,
                    brandsFilter, typesFilter, limit, currentPage, totalItems, totalPages,
                    applyFilters, changePage, debouncedSearch, onCategoryChange, onBrandChange, // [MOD] Tambah onBrandChange
                    availableCount, // [ADD] Jumlah tersedia
                    printQRItems // [ADD] Fungsi cetak QR
                };
            }
        };

        // =============== PEMINJAMAN (DITAMBAH INPUT MANUAL) ===============
        const BorrowProcess = {
            template: `
                <div>
                    ...
                    <div class="grid md:grid-cols-2 gap-6">
                        <div>
                            ...
                            <div class="bg-gray-50 p-4 rounded-lg mb-4">
                                <h3 class="font-semibold mb-2">Atau Input Manual Kode Peralatan</h3>
                                <div class="flex gap-2">
                                    <input v-model="manualBorrowCode" type="text" placeholder="Masukkan kode peralatan" class="border rounded px-3 py-2 flex-1">
                                    <button @click="addManualBorrowItem" class="bg-green-600 text-white px-4 py-2 rounded hover:bg-green-700"><i class="fas fa-plus"></i> Tambah</button>
                                </div>
                                <p v-if="manualBorrowMessage" class="mt-2 text-sm" :class="{'text-green-600': manualBorrowMessage.includes('ditambahkan'), 'text-red-600': !manualBorrowMessage.includes('ditambahkan')}">{{ manualBorrowMessage }}</p>
                            </div>
            ...
            setup() {
                ...
                // State manual
                const manualBorrowCode = app.manualBorrowCode;
                const manualBorrowMessage = app.manualBorrowMessage;
                const addManualBorrowItem = app.addManualBorrowItem;
                ...
                return { 
                    cart, guruList, rooms, selectedTeacher, selectedRoom, lastScanned, notes, 
                    startScan, removeFromCart, clearCart, processCheckout, onBorrowPhotoChange, 
                    openAddRoomModal, openAddGuruModal,
                    manualBorrowCode, manualBorrowMessage, addManualBorrowItem
                };
            }
        };

        // =============== PENGEMBALIAN (DITAMBAH INPUT MANUAL) ===============
        const ReturnProcess = {
            template: `
                <div>
                    ...
                    <div class="bg-gray-50 p-6 rounded-lg max-w-md mx-auto text-center">
                        ...
                        <div class="my-4 border-t pt-4">
                            <p class="mb-2">Atau masukkan kode peralatan secara manual:</p>
                            <div class="flex gap-2">
                                <input v-model="manualReturnCode" type="text" placeholder="Kode peralatan" class="border rounded px-3 py-2 flex-1">
                                <button @click="processManualReturn" class="bg-green-600 text-white px-4 py-2 rounded hover:bg-green-700"><i class="fas fa-undo-alt"></i> Proses</button>
                            </div>
                            <p v-if="manualReturnMessage" class="mt-2 text-sm" :class="{'text-green-600': manualReturnMessage.includes('berhasil'), 'text-red-600': !manualReturnMessage.includes('berhasil')}">{{ manualReturnMessage }}</p>
                        </div>
            ...
            setup() {
                ...
                // State manual
                const manualReturnCode = app.manualReturnCode;
                const manualReturnMessage = app.manualReturnMessage;
                const processManualReturn = app.processManualReturn;
                ...
                return { startScanReturn, message, messageClass, showUpload, onReturnPhotoChange, manualReturnCode, manualReturnMessage, processManualReturn };
            }
        };

        // =============== RIWAYAT TRANSAKSI (dengan fitur expand dan status sebagian) ===============
        const TransactionHistory = {
                ...
                <div>
                    ...
                    <div v-else class="overflow-x-auto">
                        <table class="min-w-full text-sm">
                            <thead>
                                <tr>
                                    <th class="px-3 py-2"></th> <!-- kolom untuk ikon expand -->
                                    <th class="px-3 py-2">Kode Pinjam</th>
                                    <th class="px-3 py-2">Tanggal</th>
                                    <th class="px-3 py-2">Status</th>
                                    <th v-if="isAdmin || isOperator" class="px-3 py-2">Peminjam</th>
                                    <th v-if="isAdmin || isOperator" class="px-3 py-2">Petugas</th>
                                    <th class="px-3 py-2">Ruang</th>
                                    <th class="px-3 py-2">Σ Item</th>
                                    <th class="px-3 py-2">Dikembalikan</th>
                                    <th class="px-3 py-2">Catatan</th>
                                    <th class="px-3 py-2">Bukti</th>
                                    <th v-if="isAdmin || isOperator" class="px-3 py-2">Aksi</th>
                                </tr>
                            </thead>
                            <tbody>
                                <template v-for="t in transactions" :key="t.id">
                                    <tr class="border-b hover:bg-gray-50 expandable-row" @click="toggleExpand(t.id)">
                                        <td class="px-3 py-2">
                                            <i :class="expandedRows[t.id] ? 'fas fa-chevron-down' : 'fas fa-chevron-right'"></i>
                                        </td>
                                        <td class="px-3 py-2">{{ t.borrow_code || '-' }}</td>
                                        <td class="px-3 py-2">{{ formatDateTime(t.transaction_date) }}</td>
                                        <td class="px-3 py-2">
                                            <span :class="['badge', getStatusClass(t)]">{{ getStatusText(t) }}</span>
                                        </td>
                                        <td v-if="isAdmin || isOperator" class="px-3 py-2">{{ t.borrower || '-' }}</td>
                                        <td v-if="isAdmin || isOperator" class="px-3 py-2">{{ t.admin || '-' }}</td>
                                        <td class="px-3 py-2">{{ t.room_name || '-' }}</td>
                                        <td class="px-3 py-2">{{ t.total_items || 0 }}</td>
                                        <td class="px-3 py-2">{{ t.returned_items || 0 }}</td>
                                        <td class="px-3 py-2">{{ t.notes || '-' }}</td>
                                        <td class="px-3 py-2 whitespace-nowrap">
                                            <button v-if="t.borrow_signature && t.borrow_signature !== 'dummy'" @click.stop="viewSignature(t.borrow_signature, 'Tanda Tangan Peminjaman')" class="text-blue-600 hover:text-blue-800 mr-2" title="Tanda Tangan Peminjaman"><i class="fas fa-signature"></i></button>
                                            <button v-if="t.return_signature && t.return_signature !== 'dummy'" @click.stop="viewSignature(t.return_signature, 'Tanda Tangan Pengembalian')" class="text-green-600 hover:text-green-800 mr-2" title="Tanda Tangan Pengembalian"><i class="fas fa-signature"></i></button>
                                            <button v-if="t.borrow_photo" @click.stop="viewPhoto(t.borrow_photo, 'Foto Peminjaman')" class="text-purple-600 hover:text-purple-800 mr-2" title="Foto Peminjaman"><i class="fas fa-camera"></i></button>
                                            <button v-if="t.return_photo" @click.stop="viewPhoto(t.return_photo, 'Foto Pengembalian')" class="text-orange-600 hover:text-orange-800" title="Foto Pengembalian"><i class="fas fa-camera-retro"></i></button>
                                        </td>
                                        <td v-if="isAdmin || isOperator" class="px-3 py-2 whitespace-nowrap">
                                            <!-- Tombol Edit (Detail) dan Delete -->
                                            <button @click.stop="openEditTransactionDetail(t.id)" class="text-blue-600 hover:text-blue-800 mr-2" title="Edit Transaksi"><i class="fas fa-edit"></i></button>
                                            <button @click.stop="openDeleteTransaction(t.id)" class="text-red-600 hover:text-red-800" title="Hapus Transaksi"><i class="fas fa-trash"></i></button>
                                        </td>
                                    </tr>
                                    <tr v-if="expandedRows[t.id]" class="bg-gray-50">
                                        <td colspan="14" class="px-4 py-2">
                                            <div v-if="loadingDetail[t.id]" class="text-center py-2"><i class="fas fa-spinner fa-spin"></i> Memuat...</div>
                                            <div v-else>
                                                <table class="min-w-full text-sm">
                                                    <thead>
                                                        <tr class="bg-gray-200">
                                                            ...
                                                            <th class="px-3 py-2">Tgl Kembali</th>
                                                            <th class="px-3 py-2">Tanda Tangan Kembali</th>
                                                            <th v-if="isAdmin || isOperator" class="px-3 py-2">Aksi Item</th>
                                                        </tr>
                                                    </thead>
                                                    <tbody>
                                                        <tr v-for="(item, idx) in detailItems[t.id]" :key="item.id" class="border-b">
                                                            <td class="px-3 py-2">{{ idx+1 }}</td>
                                                            <td class="px-3 py-2">{{ item.item_code }}</td>
                                                            <td class="px-3 py-2">{{ item.item_name }}</td>
                                                            <td class="px-3 py-2"><span :class="['badge', item.status === 'returned' ? 'badge-success' : 'badge-warning']">{{ item.status === 'returned' ? 'Dikembalikan' : 'Dipinjam' }}</span></td>
                                                            <td class="px-3 py-2">{{ formatDate(item.borrow_date) }}</td>
                                                            <td class="px-3 py-2">{{ formatDate(item.return_date) }}</td>
                                                            <td class="px-3 py-2">
                                                                <img v-if="item.return_signature && item.return_signature !== 'dummy'" :src="item.return_signature" class="signature-img" />
                                                                <span v-else>-</span>
                                                            </td>
                                                            <td v-if="isAdmin || isOperator" class="px-3 py-2 whitespace-nowrap">
                                                                <button v-if="item.status === 'borrowed'" @click.stop="returnTransactionItem(item.id)" class="text-green-600 hover:text-green-800 mr-2" title="Kembalikan Item"><i class="fas fa-undo-alt"></i></button>
                                                                <button v-if="item.status === 'returned'" @click.stop="revertTransactionItem(item.id)" class="text-yellow-600 hover:text-yellow-800 mr-2" title="Batalkan Pengembalian (Kembalikan ke Dipinjam)"><i class="fas fa-rotate-left"></i></button>
                                                                <button @click.stop="openEditTransactionItem(item)" class="text-blue-600 hover:text-blue-800 mr-2" title="Edit Item"><i class="fas fa-edit"></i></button>
                                                                <button @click.stop="deleteTransactionItem(item.id)" class="text-red-600 hover:text-red-800" title="Hapus Item"><i class="fas fa-trash"></i></button>
                                                            </td>
                                                        </tr>
                                                    </tbody>
                                                </table>
            ...
            setup() {
                ...
                const returnTransactionItem = app.returnTransactionItem;
                const revertTransactionItem = app.revertTransactionItem;
                const deleteTransactionItem = app.deleteTransactionItem;
                const openEditTransactionItem = app.openEditTransactionItem;
                const formatIndonesianDate = app.formatIndonesianDate;
                
                const transactions = ref([]);
                const loading = ref(true);
                const showSignatureViewModal = ref(false);
                const signatureViewData = ref('');
                const signatureViewTitle = ref('');
                const filterStatus = ref('all');
                const dateFrom = ref('');
                const dateTo = ref('');
                ...
                function getStatusClass(t) {
                    if (t.returned_items == t.total_items) return 'badge-success';
                    if (t.returned_items == 0) return 'badge-warning';
                    return 'badge-gray';
                }
                function getStatusText(t) {
                    if (t.returned_items == t.total_items) return 'Dikembalikan';
                    if (t.returned_items == 0) return 'Dipinjam';
                ...
                function formatDateTime(d) {
                    return formatIndonesianDate(d, true);
                }
                ...
                async function toggleExpand(transId) {
                    if (expandedRows.value[transId]) {
                        // tutup
                        expandedRows.value[transId] = false;
                    } else {
                        // buka
                        expandedRows.value[transId] = true;
                        if (!detailItems.value[transId]) {
                            loadingDetail.value[transId] = true;
                            try {
                                const res = await fetch(`?action=getTransactionDetail&id=${transId}`);
                                const data = await res.json();
                                detailItems.value[transId] = data.items || [];
                            } catch (e) {
                                console.error(e);
                                detailItems.value[transId] = [];
                            } finally {
                                loadingDetail.value[transId] = false;
                            }
                        }
                    }
                }
                ...
                return { 
                    isAdmin, isOperator, transactions, loading, formatDate, formatDateTime, viewSignature, viewPhoto, 
                    showSignatureViewModal, signatureViewData, signatureViewTitle, 
                    filterStatus, dateFrom, dateTo, filterUser, guruList, applyFilter, resetFilter, 
                    openEditTransactionDetail, openDeleteTransaction,
                    returnTransactionItem, revertTransactionItem, deleteTransactionItem, openEditTransactionItem,
                    expandedRows, detailItems, loadingDetail, toggleExpand,
                    ...
                    <div v-else class="space-y-6">
                        <div v-for="t in transactions" :key="t.id" class="border rounded-lg p-4 bg-white shadow-sm">
                            <div class="flex flex-wrap gap-4 items-center mb-3 pb-2 border-b">
                                ...
                                <div><span class="font-semibold">Status:</span> <span :class="['badge', t.status === 'returned' ? 'badge-success' : 'badge-warning']">{{ t.status === 'returned' ? 'Dikembalikan' : 'Dipinjam' }}</span></div>
                            </div>
        ...
        // =============== LAPORAN (dengan gambar paraf) ===============
        const Report = {
            template: `
                <div>
                    <h2 class="text-2xl font-bold mb-5">Laporan Peminjaman Peralatan</h2>
                    
                    <!-- Tab Navigation dengan style baru (hanya dua tab) -->
                    <div class="mb-6 flex flex-wrap gap-2">
                        <button @click="activeTab = 'monthly'" :class="['tab-button', activeTab === 'monthly' ? 'tab-button-monthly-active' : 'tab-button-inactive']">
                            CETAK LAPORAN
                        </button> | 
                        <button @click="activeTab = 'bycode'" :class="['tab-button', activeTab === 'bycode' ? 'tab-button-bycode-active' : 'tab-button-inactive']">
                            LAPORAN KODE PINJAM
                        </button>
                    </div>
                    ...
                    <div v-if="activeTab === 'monthly'">
                        <!-- Konten laporan bulanan dengan pengelompokan berdasarkan KODE PINJAM dan status grup -->
                        ...
                        <div v-if="loading" class="text-center py-5"><i class="fas fa-spinner fa-spin"></i></div>
                        <div v-else-if="groupedData.length === 0" class="text-center py-5 text-gray-500">Tidak ada data untuk bulan ini.</div>
                        <div v-else id="print-area" class="bg-white p-6 rounded-lg shadow overflow-x-auto">
                            <div class="text-center mb-6">
                                ...
                                <table class="min-w-full text-sm border border-gray-300">
                                    <thead>
                                        <tr class="bg-gray-100">
                                            ...
                                            <th class="border px-3 py-2" rowspan="2">KODE PINJAM</th>
                                            <th class="border px-3 py-2" rowspan="2">NAMA PEMINJAM</th>
                                            <th class="border px-3 py-2" rowspan="2">TANGGAL</th>
                                            <th class="border px-3 py-2" rowspan="2">RUANG</th>
                                            <th class="border px-3 py-2" rowspan="2">STATUS</th>
                                            <th class="border px-3 py-2" colspan="2">PERALATAN</th>
                                            <th class="border px-3 py-2" colspan="2">PARAF</th>
                                            <th class="border px-3 py-2" rowspan="2">KETERANGAN</th>
                                        </tr>
                                        <tr class="bg-gray-50">
                                            <th class="border px-3 py-2">KODE</th>
                                            <th class="border px-3 py-2">ITEM</th>
                                            <th class="border px-3 py-2">PINJAM</th>
                                            <th class="border px-3 py-2">KEMBALI</th>
                                        </tr>
                                    </thead>
                                    <tbody>
                                        <template v-for="(group, index) in groupedData" :key="group.borrow_code">
                                            <tr v-for="(item, idx) in group.items" :key="item.item_code + idx">
                                                <td v-if="idx === 0" :rowspan="group.items.length" class="border px-3 py-2 text-center align-middle">{{ index + 1 }}</td>
                                                <td v-if="idx === 0" :rowspan="group.items.length" class="border px-3 py-2 align-middle">{{ group.borrow_code }}</td>
                                                <td v-if="idx === 0" :rowspan="group.items.length" class="border px-3 py-2 align-middle">{{ group.peminjam }}</td>
                                                <td v-if="idx === 0" :rowspan="group.items.length" class="border px-3 py-2 align-middle">{{ group.tanggal }}</td>
                                                <td v-if="idx === 0" :rowspan="group.items.length" class="border px-3 py-2 align-middle">{{ group.ruang }}</td>
                                                <td v-if="idx === 0" :rowspan="group.items.length" class="border px-3 py-2 align-middle">
                                                    <span :class="['badge', group.group_status_class.replace('bg-','').replace('text-','')]">{{ group.group_status }}</span>
                                                </td>
                                                <td class="border px-3 py-2">{{ item.item_code }}</td>
                                                <td class="border px-3 py-2">{{ item.item_name }}</td>
                                                <td class="border px-3 py-2 text-center">
                                                    <img v-if="item.borrow_signature && item.borrow_signature !== 'dummy'" :src="item.borrow_signature" class="signature-img" />
                                                    <span v-else>-</span>
                                                </td>
                                                <td class="border px-3 py-2 text-center">
                                                    <img v-if="item.return_signature && item.return_signature !== 'dummy'" :src="item.return_signature" class="signature-img" />
                    ...
                    <div v-else-if="activeTab === 'bycode'">
                        <ReportByCode />
                    </div>
                </div><div style="margin-bottom:70px;"></div>
            `,
            setup() {
                ...
                const years = ref([]);
                const data = ref([]); // data mentah dari API
                const groupedData = ref([]); // data setelah dikelompokkan
                ...
                async function fetchReport() { 
                    ...
                    groupedData.value = result.data; // data sudah dikelompokkan dari server
                    loading.value = false; 
                }
                ...
                return { month, year, years, monthName, currentDate, data, groupedData, loading, fetchReport, printReport, activeTab };
            }
        };
